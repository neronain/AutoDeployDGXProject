...
def burn(
    ...
    **ต้องมี torch ที่เห็น CUDA อยู่บนเครื่องนั้น** — หาจาก interpreter ก่อน แล้วถอยไปใช้
    image ที่มีอยู่ในเครื่องแล้ว (ไม่ pull ให้ · ไซต์ที่ตัดขาดอินเทอร์เน็ตต้องใช้ได้)

    ในทางปฏิบัติแปลว่า **เครื่องที่รัน llama.cpp อย่างเดียวมักวัดไม่ได้** เพราะไม่เคยต้องมี
    image ของ vLLM · วัดจริงบนฟลีต 16 เครื่อง (2026-09-21): มี torch+CUDA บนเครื่องจริง
    เครื่องเดียว ที่เหลือวัดได้เพราะยืม image ของ vLLM และ 3 เครื่องวัดไม่ได้เลยเพราะไม่มีทั้งคู่
    · ทางแก้ที่ไม่ต้องต่อเน็ต: `docker save` จากเครื่องข้าง ๆ แล้ว `docker load` หรือชี้เอง
    ด้วย `LMDS_BURN_PYTHON` / `LMDS_BURN_IMAGE`
    """
    from lmds.hardware import burn as burn_check
