...
# ลายเซ็นของ process นี้ — สุ่มใหม่ทุกครั้งที่ import ซึ่งคือทุกครั้งที่บริการเริ่มใหม่
# หน้าเว็บเทียบค่านี้เพื่อรู้ว่า restart เสร็จแล้วจริง โดยไม่ต้องพึ่งว่า commit เปลี่ยนไหม
...
def create_app(token: str = "") -> FastAPI:
    ...
    def version(check_repo: bool = False) -> dict:
        ...
        payload = {
            ...
            # ลายเซ็นของ *process นี้* — เปลี่ยนทุกครั้งที่บริการเริ่มใหม่
            #
            # หน้าเว็บใช้ตอบคำถาม "restart เสร็จหรือยัง" ซึ่งเดิมถามผ่าน commit แล้วผิด
            # เมื่ออัปเดตแล้วไม่มีอะไรเปลี่ยน (ดึงมาแล้วเป็นของเดิม): commit ไม่ขยับ
            # หน้าเว็บเลยรอจนหมดเวลาแล้วสรุปว่าเซิร์ฟเวอร์ไม่กลับมา ทั้งที่กลับมาตั้งนานแล้ว
            # — และหยุดก่อนไล่อัปเดต node ที่เหลือ ซึ่งคือทั้งหมดที่ผู้ใช้กดปุ่มมาเพื่อทำ
            "boot": _BOOT_ID,
            "upstream": "",
    ...
    def node_controller_command(name: str, slug: str, command: str) -> dict:
        ...
        allowed = {
            "test-text", "test-vision", "test-reasoning", "test-tools", "test-embed", "test-rerank",
