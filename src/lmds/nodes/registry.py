...
class Node:
    ...
    # ไม่แตะการจับกลุ่ม stacked เลย (ดู `stack`/`cluster_ip`) · ว่าง = ยังไม่จัดกลุ่ม ซึ่งทำให้
    # ทะเบียนเดิมทั้งหมดอยู่กลุ่มเดียวกันเหมือนก่อนมีฟิลด์นี้ · stacked ทำได้เฉพาะเครื่อง
    # site เดียวกันอยู่แล้วโดยธรรมชาติ (สายเร็วต่อถึงกันได้เฉพาะในที่เดียวกัน) — ฟิลด์นี้
    # แค่ทำให้ "เห็น" การแบ่งนั้น ไม่ได้เป็นตัวบังคับ
    site: str = ""
