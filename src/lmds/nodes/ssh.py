...
#
# repo เป็น public แล้ว · เดิมเป็น private ทุกเครื่องที่เพิ่มเข้าฟลีตจึงต้องมี deploy key ของ
# ตัวเองก่อน ไม่งั้น "could not read Username" — คือขั้นที่ยุ่งยากที่สุดของการติดตั้ง และเป็น
# เหตุผลที่เคยต้องส่ง bundle ด้วยมือ (13 ส.ค. 69) · ตอนนี้ clone ตรงได้แล้ว แต่ทางนี้ยังจำเป็น:
# เครื่องที่ออกเน็ตไม่ได้ติดตั้งได้ทางนี้ทางเดียว · hub มี checkout อยู่แล้ว ส่งไปเองได้ (~2 MB)
