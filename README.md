...
[![tests](https://img.shields.io/badge/tests-2451-17703f)](tests/)
...
|  |  |
|---|---|
| **มันคืออะไร** | ให้ลิงก์โมเดลจาก Hugging Face แล้วได้เซิร์ฟเวอร์ OpenAI-compatible ที่รันอยู่บนเครื่องของคุณ พร้อมคอนโซลเว็บสำหรับคุมทุกเครื่องในฟลีต |
| **มันแก้อะไร** | คำสั่งที่ดูถูกทุกอย่างแต่ให้ผลผิดเงียบ ๆ — context ถูกตัดเหลือหนึ่งในสิบ, tool calling ที่ไม่เคยแปลงคำตอบ, สาย 200G ที่ลงมาเหลือ 50G, KV cache ที่คำนวณเกินจริงยี่สิบเท่า |
| **เริ่มยังไง** | `./install.sh -y` แล้ว `lmds web --enable --bind 0.0.0.0` — เครื่องที่เหลือกด *Add machine* บนหน้าเว็บ ไม่ต้องไปติดตั้งทีละเครื่อง |
| **ราคา** | **1 เครื่อง = ฟรีตลอด** ใช้เชิงพาณิชย์ได้ · ตั้งแต่ 2 เครื่องที่ดูแลร่วมกันต้องมีไลเซนส์ |
| **ข้อมูลของคุณ** | **ไม่มี telemetry** · ใช้ได้แบบ air-gapped · หน้าเว็บไม่ดึงอะไรจากอินเทอร์เน็ตเลยแม้แต่ฟอนต์ |
...
กลับให้ผลผิดโดยไม่มี error** · LMDS เกิดจากการไล่รันของจริงแล้วเก็บทุกอาการพวกนี้กลับมาเป็นการตรวจอัตโนมัติ
...
โมเดล gated (เช่น `meta-llama/Llama-3.3-70B-Instruct`) ระบบถาม HF token ให้เอง กด Enter ข้ามได้
...
(โมเดลทั้งฟลีต / คะแนน / สูตร / weights / ตั้งค่า hub) · สลับ **อังกฤษ ↔ ไทย** ได้จากปุ่มบนหัวหน้า
...
autostart · คำสั่ง stacked · repair · remove · **ตั้ง/สุ่ม/ถอน API key ของโมเดล** · **เปลี่ยน hostname
ของเครื่อง** · ปุ่ม **Update** ทั้งฟลีต — **และคุมโมเดลบนเครื่องอื่นได้เท่ากับเครื่องตัวเอง**

<div align="center">

<img src="docs/img/machine.png" alt="หน้ารายละเอียดเครื่อง — เกจและสายเชื่อม" width="900">
...
มันรู้กติกาเรื่อง context/KV แต่**ถูกสั่งห้ามคิดเลขเอง** เพราะเลขที่ LLM คูณเองผิดแบบดูน่าเชื่อ ซึ่งแย่กว่าตอบว่าไม่รู้ ·
เมื่อสาเหตุชัดพอจะเสนอวิธีแก้ มันจะ**ถามกลับเป็นเมนู** (*แก้เลย* / *ทีละขั้น* / *ยังไม่ทำ*) แทนที่จะลงมือเอง —
**LLM สั่งงานเองไม่ได้**: มันเลือกได้แค่ชื่อรายการจากแคตตาล็อกที่กำหนดไว้ คำสั่งจริงประกอบด้วยโค้ด
และตั๋วอนุมัติออกโดยเซิร์ฟเวอร์ ทางเดียวที่คำสั่งจะทำงานคือมีคนกดปุ่ม

#### คะแนนที่วัดจากเซิร์ฟเวอร์จริง

<div align="center">

<img src="docs/img/model-scores.png" alt="คะแนนโมเดลที่วัดจากเซิร์ฟเวอร์จริง" width="900">

*ยิงผ่าน OpenAI API ของเซิร์ฟเวอร์ที่รันอยู่จริง — decode / prefill / TTFT แยกตามงาน 6 แบบ
และความสามารถ 7 ข้อที่ทดสอบทีละข้อ (instructions · Thai · JSON · tools · reasoning · vision · recall)
· ในภาพ: Qwen3.6-35B-A3B (Q4_K) บน 3× RTX 3060 ได้ 65.7–84.4 tok/s ที่ context 262,144
— เทียบข้าม engine และข้ามเครื่องได้* · [BENCH.md](docs/BENCH.md)
...
**ทั้งสองอย่างนี้ทำจากหน้าเว็บได้แล้ว** — ปุ่ม **Key** บนการ์ดโมเดล (สุ่มใหม่ · ดูค่าเต็ม · วางของเดิม · ถอนออก)
และแผง **"ใครสั่งอะไร"** ในหน้า *Hub settings* (กรองเฉพาะที่ถูกปฏิเสธได้) · เพิ่มเพราะ `lmds key` กับ
`lmds audit` มีแต่ทาง CLI ขณะที่ลูกค้าส่วนใหญ่ใช้ GUI อย่างเดียว — wizard บนหน้าเว็บเองยังเคยพิมพ์บอกให้ไปรัน
`lmds key show <slug> --reveal` ซึ่งเป็นคำสั่งที่คนกลุ่มนั้นพิมพ์ไม่ได้อยู่แล้ว · "ดูค่าเต็ม" แยกเป็นคนละ
endpoint จากการอ่านสถานะ เพื่อไม่ให้ key เดินทางผ่านเครือข่ายทุกครั้งที่หน้าเว็บ poll
...
lmds node rename-host msi-6 spark-7                # เปลี่ยน hostname ของ OS บนเครื่องนั้น
```
...
### เปลี่ยน hostname ของเครื่องจากหน้าเว็บ

เครื่องที่ส่งออกไปอยู่กับลูกค้าแล้วพบว่า **ตั้ง hostname ซ้ำกัน** แก้เองไม่ได้ เพราะทางเข้าเหลือ
**คอนโซล LMDS (พอร์ต 8600) ทางเดียว** ไม่มี SSH ให้ใครเข้าไปพิมพ์ `hostnamectl` — งานนี้จึงมี
**ปุ่ม Rename host บนการ์ดเครื่อง** เป็นของหลัก (CLI คือ `lmds node rename-host <ชื่อ> <hostname>`)

- เขียน `hostnamectl set-hostname` + บรรทัด `127.0.1.1` ใน `/etc/hosts` **ในคำสั่ง sudo เดียวกัน** —
  แก้อย่างแรกแล้วลืมอย่างที่สองคือ `sudo` ทุกครั้งถัดไปช้าลงเป็นวินาทีและขึ้น "unable to resolve host"
- ตรวจ RFC 1123 · สำรองไฟล์เดิมไว้ที่ `/root/lmds-hostname/*.<stamp>` บนเครื่องนั้น (ไม่ลบทิ้ง) ·
  รหัส sudo เดินทางทาง **stdin เท่านั้น** ไม่อยู่ใน argv / log / ทะเบียน
- **ปฏิเสธ ไม่ทำครึ่งเดียว**: ถ้า hub เข้าเครื่องนี้ด้วยชื่อที่กำลังจะเปลี่ยน (เปลี่ยนเสร็จแล้วเข้าไม่ได้อีก
  และไม่มี SSH ให้ไปแก้คืน) · ถ้ามีโมเดล **stacked** รันอยู่ · หรือชื่อใหม่ชนกับเครื่องอื่น

> **ชื่อในทะเบียนกับ hostname ของ OS เป็นคนละอย่างโดยตั้งใจ** — ชื่อในทะเบียนคือป้ายที่ทุกคำสั่งและทุกปุ่ม
> ใช้เรียกเครื่อง คำสั่งนี้เปลี่ยนเฉพาะชื่อที่ *ตัวเครื่อง* เรียกตัวเอง · **ไม่เกี่ยวกับการนับเครื่องตามไลเซนส์**
> เพราะตัวนับผูก hostname เข้ากับชุด IP บนสายเร็วอยู่แล้ว เครื่องคนละตัวที่ชื่อซ้ำกันถูกนับแยกถูกต้องมาตั้งแต่ต้น ·
> `~/.lmds/bench/*.json` และ PROFILE.yaml ของสูตรจดชื่อ ณ วันที่วัดไว้เป็นหลักฐาน **ตั้งใจไม่แก้ย้อนหลัง**
...
lmds bench run <ชื่อ>     # วัดความเร็ว + ความสามารถ แล้วเก็บไว้เทียบ (ดู BENCH.md)
...
`lmds burn` ใส่โหลดเต็ม GPU 15 วินาทีแล้ววัด — เพราะค่าที่อ่านตอน GPU ว่าง เครื่องดีกับเครื่องที่ถูกล็อก
ให้ตัวเลขต่ำเหมือนกันหมด · **แยกสามสภาพที่ห้ามปนกัน**: คล็อกต่ำ + ไฟต่ำ = ล็อกจริง → บอก**ขั้นตอน
ทางกายภาพ** (ปิดเครื่อง → ถอดปลั๊ก adapter 30–60 วินาที → เสียบกลับ) เพราะซ่อมจากระยะไกลไม่ได้ ·
คล็อกต่ำ + มี clock event reason = ความร้อน/ไฟ ต้องไปดูลมกับไฟ ไม่ใช่ถอดปลั๊ก · คล็อกปกติ + TFLOPS ต่ำ
= มีคนอื่นใช้ GPU อยู่ ไม่ใช่ความผิดของเครื่อง

`lmds bench run` เรียกให้อัตโนมัติก่อนวัดทุกครั้ง แล้ว **เก็บผลไปกับตัวเลข** เพื่อให้รู้ทีหลังว่ารอบนั้นคล็อกจริง
หรือเปล่า (ข้าม: `--skip-burn`) · เครื่องที่ไม่ใช่ GB10 / ไม่มี NVIDIA ตอบว่า "ไม่เกี่ยว" ไม่ใช่ "ตก" · `--node <ชื่อ>`
ตรวจข้ามเครื่อง `--all` ตรวจทั้งฟลีต โดย**ส่งสคริปต์ไปรัน ไม่ได้เรียก `lmds` บน node** — เครื่องที่ยังไม่เคย
ติดตั้ง LMDS หรือมีรุ่นเก่ากว่า hub ก็ตรวจได้ และไม่ดึงอะไรจากเน็ต

> **ข้อจำกัดที่ต้องรู้ก่อน** · การวัดต้องใช้ torch ที่เห็น CUDA บนเครื่องนั้น — หาจาก interpreter ก่อน
> แล้วถอยไปใช้ image ที่มีอยู่ในเครื่องแล้ว (**ไม่ pull ให้** ไซต์ที่ตัดขาดอินเทอร์เน็ตต้องใช้ได้)
> · แปลว่า **เครื่องที่รัน llama.cpp อย่างเดียวมักวัดไม่ได้** เพราะไม่เคยต้องมี image ของ vLLM
> · วัดจริงบนฟลีตของเรา 16 เครื่อง (2026-09-21): มี torch+CUDA บนเครื่องจริงแค่เครื่องเดียว ·
> ที่เหลือวัดได้เพราะยืม image ของ vLLM · **3 เครื่องวัดไม่ได้เลย**
> · เครื่องแบบนั้นได้ผลเป็น "ตรวจไม่ได้" ไม่ใช่ "ตก" พร้อมทางแก้ — ก๊อป image จากเครื่องข้าง ๆ
> ด้วย `docker save` → `docker load` (ไม่ต้องต่อเน็ต) หรือชี้เองด้วย `LMDS_BURN_PYTHON` / `LMDS_BURN_IMAGE`

### `lmds watchdog` — `/health` เขียวได้ทั้งที่ rank ค้าง

`/health` ตอบ 200 ได้โดยที่ forward pass ไม่เดิน · สิ่งเดียวที่พิสูจน์ได้ว่ามันยังเดินคือ**สั่ง generate
แล้วได้ token กลับมา** · watchdog ยิง generate จริง **2 token** ทุก 120 วินาที แล้ว restart ให้เมื่อ
พลาดติดกันหลายรอบ (ไม่ใช่รอบเดียว — GC pause กับเน็ตสะดุดมีจริง)

**ต้องสั่งเปิดเองทีละตัว ไม่มีอะไรเปิดเอง** — เพราะ LMDS ตั้งใจให้คุมเครื่องที่มีโมเดลของลูกค้ารันอยู่ก่อนแล้ว
การมีอะไรในเครื่องที่ restart โมเดลของลูกค้าได้เองเป็นเรื่องใหญ่กว่าการปล่อยให้ค้างแล้วมีคนมาเห็น · ตามมาด้วย:

- **ปฏิเสธตั้งแต่ตอน arm**: container ที่ LMDS ไม่ได้สร้าง (`lmds adopt`) · ตัวที่ไม่มีทะเบียน ·
  โมเดล embedding/rerank (ยิง generate ใส่มันไม่มีความหมาย ผลคือ restart เพราะเราถามผิด)
- **เพดานจำนวน restart ต่อกรอบเวลา** แล้ว**เลิก restart ถาวร** — ยังตรวจต่อ ยังรายงานต่อ แต่ไม่ restart อีก ·
  โมเดลที่ start ไม่ขึ้นแล้วถูก restart ทุก 2 นาทีตลอดคืนคือความเสียหายที่ watchdog สร้างขึ้นเอง ·
  มี **backoff** ระหว่างครั้ง และ **ช่วงพักหลัง restart** (โมเดลใหญ่โหลดเป็นสิบนาที)
- **4xx = ยังไม่ตาย** — 404/401 แปลว่ามันตอบอยู่ รายงานว่า `misconfigured` ให้คนไปแก้ probe ไม่ใช่รีสตาร์ตโมเดล
- ทุก restart ลง `lmds audit` พร้อมเหตุผล — คำตอบ "ระบบสั่งเอง" ต้องหาเจอที่เดียวกับคำสั่งของคน ·
...
**24 target preset** (7 ตัวทดสอบบนเครื่องจริงแล้ว) · **2,451 เทสต์** รันครบทุก push บน Python 3.10–3.13
...
สิทธิ์ต่อคน และตรวจว่าเซิร์ฟเวอร์ที่รันอยู่**ทำอะไรได้จริง** · **ไม่มีตัวไหนต้องพึ่งอีกตัว** — parser ที่
LiteGate บอกว่าขาดคือ knob ที่ LMDS เปิดได้ทันทีด้วย `restart --tool-parser` แล้วพิสูจน์ด้วย `test-tools`
ซึ่งวัดโหมด `auto` — โหมดเดียวกับที่ agent ใช้จริง
...
| [DGX-SPARK-VLLM-FIELD-NOTES.md](docs/DGX-SPARK-VLLM-FIELD-NOTES.md) · [HCA-DUAL-TEST.md](docs/HCA-DUAL-TEST.md) | บันทึกภาคสนามของจริง — ทุกบรรทัดมีระดับความเชื่อถือกำกับ |
| [GPU-UTIL-RANK-PROTOCOL.md](docs/GPU-UTIL-RANK-PROTOCOL.md) | โปรโตคอลวัดหน่วยความจำนอกงบของ vLLM ว่าโตตามจำนวน rank ไหม (`scripts/gpu_util_rank_probe.py`) — **เขียนเสร็จแล้ว ยังไม่ได้รัน ยังไม่มีตัวเลข** |
