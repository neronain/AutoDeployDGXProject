...
*16 เครื่อง · 7 GPU · 688 GB VRAM · 21 โมเดลที่รันอยู่ — หน้าเดียว · แถบหน่วยความจำเรียงจาก
เครื่องที่เต็มที่สุด และ **Needs attention** ที่คำนวณจากของจริง (ในภาพ: สองเครื่องมีโมเดล
หลายตัวจองพอร์ต 8000 ตัวเดียวกัน ตัวที่หยุดอยู่จึง start ไม่ขึ้นจนกว่าจะเปลี่ยนพอร์ต)*
...
GB10 พร้อมคล็อกและอุณหภูมิ, ทุก IP ที่เครื่องนั้นถืออยู่จริง, สาย **200G RDMA** และคู่
`CLUSTER A` ที่ stacked กันได้ · ค่าที่การ์ดไม่รายงานถูกซ่อน ไม่ใช่โชว์ 0*

<img src="docs/img/model-scores.png" alt="คะแนนโมเดลที่วัดจากเซิร์ฟเวอร์จริง" width="900">

*คะแนนที่ยิงผ่าน OpenAI API ของเซิร์ฟเวอร์จริง — decode / prefill / TTFT แยกตามงาน 6 แบบ
และความสามารถ 7 ข้อที่ทดสอบทีละข้อ · ในภาพ: Qwen3.6-35B-A3B (Q4_K) บน 3× RTX 3060
ได้ 65.7–84.4 tok/s ที่ context 262,144 — เทียบข้าม engine และข้ามเครื่องได้*
