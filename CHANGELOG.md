...
- **หน้าเว็บติดตั้ง LMDS บนเครื่องอื่นได้เอง** — เครื่องที่ยังไม่มี `lmds` ขึ้นว่า Unreachable
  ตลอดเพราะ hub เรียก `lmds agent info` บนเครื่องนั้น ไม่ได้ส่ง agent ไปรัน · เดิมต้อง ssh
  เข้าไปติดตั้งเอง ซึ่งขัดกับเหตุผลที่มีหน้าเว็บ · ปุ่ม **install** อยู่ทั้งบนหัวแถวและในกล่อง
  Unreachable · เป็น job เพราะใช้เวลาหลายนาที และงานซ้อนกันบนเครื่องเดียวถูกปฏิเสธ (409)
  เพราะ `git pull` ซ้อน `git pull` ทำ repo พัง · ขั้น Docker/toolkit ถูกข้ามเสมอ (ต้องใช้ sudo
  ซึ่งหน้าเว็บไม่มี tty ให้กรอก)
- **แก้ที่อยู่สำรองของเครื่องจากหน้าเว็บ** (`alt_hosts`) — เท่ากับ `lmds node set --alt-host`
- **กล่องโต้ตอบของเราเองแทน `alert`/`confirm`/`prompt`** — ของเดิมบล็อกทั้งหน้า จัดสไตล์ไม่ได้
  และบนมือถือขึ้นชื่อโดเมนนำหน้าจนอ่านไม่ออกว่าใครถาม · ใช้ `<dialog>` ซึ่งให้ focus trap,
  Esc ปิด, `inert` ข้างหลัง และ `::backdrop` มาตามมาตรฐาน · log ที่เคยยัดใน `alert` อยู่ใน
  `<pre>` ที่เลื่อนได้แล้ว · มีเทสกันไม่ให้เผลอกลับไปใช้ของเดิม

### Changed

- **จัด layout หน้าเว็บใหม่ทั้งหน้าให้ใช้ได้จริงทั้งมือถือและเดสก์ท็อป**
  · ระยะห่างเป็นสเกลเดียวกันทั้งหน้า (`--s1`..`--s6`) แทนตัวเลขที่นึกออกตอนนั้น —
  จุดที่แน่นจนอ่านยาก (stat กับการ์ดถัดไปห่างกัน 4px, การ์ดวิซาร์ดชนการ์ดเครื่อง) หายไป
  · ปุ่มกับช่องกรอกสูงเท่ากันทุกที่ผ่าน `--control` ตัวเดียว (เดิมต่างคนต่างตั้ง padding
  แล้วไม่เท่ากันทุกครั้งที่วางเรียงกัน)
  · แถวสูตรและแถวเครื่องในหน้า cluster เปลี่ยนจาก flex เป็น **grid** — คอลัมน์ตรงกันทุกแถว
  เวลากวาดสายตาลงมา ของเดิมแต่ละแถวจัดคนละที่ตามความยาวข้อความ
  · **มือถือ**: ปุ่ม Deploy ย้ายมาเป็นปุ่มลอยล่างในระยะนิ้วโป้ง · ทุก control สูง 40px ·
  ช่องกรอก 16px (iOS ไม่ซูมเข้าตอนโฟกัส) · ไม่มีการเลื่อนแนวนอนแล้ว
  (วัดที่ 375px: เดิม `scrollWidth` 411 ตอนนี้ 375 · ปุ่มที่เล็กกว่า 36px เดิม 14 ตัว ตอนนี้ 0)
  · **เดสก์ท็อป**: หน้ากว้างขึ้นเป็น 1320px และแผงที่เป็นอิสระต่อกัน (weights / recipes)
  วางคู่กันบนจอ ≥1100px แทนที่จะเรียงลงมายาว ๆ โดยปล่อยครึ่งจอขวาว่าง
  · แถบสัญญาณนาฬิกา GPU หนาขึ้นและ**บอกเพดานกับ %** — "208 MHz" เฉย ๆ บอกไม่ได้ว่า
  การ์ดว่างหรือคอขวด
  · รายการสูตรเดิมเป็นกำแพงชิปของ *ชื่อคีย์* ที่ตั้ง (ไม่ใช่ค่า) กินครึ่งจอ — ตอนนี้บรรทัดเดียว
  ต่อสูตร กางดูค่าจริงได้
  · รายการ target 20 กว่าตัวเรียงพรืด — จัดกลุ่มด้วย `<optgroup>` ตามตระกูลเครื่อง
  พร้อมขนาดหน่วยความจำ และเรียงตัวที่ทดสอบแล้วขึ้นก่อน
  · เข้าถึงได้ตามมาตรฐาน: `<main>`/`<section>`/`<h2>` จริง, skip link, `aria-busy`,
  `role="meter"` บนแถบสัญญาณ, และแก้สีหัวข้อที่ contrast 3.1:1 (ตกเกณฑ์ WCAG AA)

### Fixed

- **ขอบเขตความปลอดภัยของหน้าเว็บ one-stop** — escape ค่าจาก model/path/hostname/job log
  ก่อนเขียนลง DOM และจำกัดลิงก์ภายนอกเป็น `http(s)` พร้อม CSP nonce · API ที่เปลี่ยนสถานะ
  ปฏิเสธ cross-origin request และโหมดไม่มี token รับเฉพาะ Host บน loopback · การลบโมเดล/
  เครื่องต้องยืนยันชื่อให้ตรง, input ของ SSH/options ผ่าน allowlist+type/range check ก่อน side effect
  · start/restart ส่ง env ให้ subprocess โดยตรง ไม่แก้ `os.environ` ร่วมของ web process
- **ระบบเสนอ cluster IP ของ hub เองแล้วไปคนละวงกับเครื่องอื่น แล้วฟ้อง split-fabric
  ที่ตัวเองสร้าง** — DGX Spark มี fabric สองวงที่เร็วเท่ากัน (คู่แฝดของพอร์ตเดียวกัน) และทุก
  เครื่องมักมีขาอยู่ทั้งสองวง การเลือกตามความเร็วอย่างเดียวจึงตัดสินด้วยลำดับที่อ่านเจอ
  · เคสจริงบนสองเครื่อง: s2 ตั้ง `10.100.16.2` ไว้ แต่ hub ถูกเสนอ `10.100.17.1`
  แล้วขึ้นว่า "cluster IP อยู่คนละวง ต่อกันไม่ติด" ทั้งที่ผู้ใช้ไม่ได้ทำอะไรผิดเลย
  · ตอนนี้เลือกวงที่เครื่องอื่น**ตั้งไว้จริง**ก่อนความเร็วเสมอ
- **หน้าเว็บมีเทสว่าสคริปต์ทั้งหน้า parse ได้** — สคริปต์อยู่ในไฟล์เดียว พิมพ์ผิดจุดเดียว
  คือทั้งหน้าตาย ไม่ใช่แค่ฟีเจอร์เดียว · เจอจริงตอนเขียนรอบนี้: template literal ที่ควรเป็น
  `
` กลายเป็นขึ้นบรรทัดใหม่จริง ผลคือ "Other machines" ค้างที่ Loading… ตลอดกาล
  โดยเทสฝั่ง API ผ่านหมด — เบราว์เซอร์ต่างหากที่พัง
...
- **`lmds web` — หน้าเว็บคุมโมเดล (เฟส 2 เริ่มแล้ว)** · หน้าเดียว: สถานะเครื่อง (unified memory
  ...
  - **ปิดช่องว่างงานประจำเทียบ CLI** (รายงานจากผู้ใช้: "ถ้า feature ไม่ครบเหมือน CLI จะมี GUI ไปทำไม")
    ...
    · ไม่ได้หมายถึงทุกคำสั่งของ CLI (`config`/`hardware` ยังอยู่ที่ CLI) · autostart ต้องใช้ sudo
    ซึ่งเว็บไม่มี tty — ล้มเหลวแล้วส่งคำสั่งกลับไปให้รันเอง ไม่ใช่ 500 เปล่า
