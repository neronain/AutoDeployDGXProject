...
- **`lmds up <ลิงก์>` — ลิงก์เดียวจบถึงเซิร์ฟเวอร์ตอบได้จริง** · `lmds deploy` หยุดที่ bundle
  ...
  · จบแล้วบอกวิธีต่อ client ต่อทันที (`lmds connect`, `client-config`)
  · **stacked ไม่ทำแทน** — ขั้น sync-worker/verify-worker ต้องตัดสินใจเรื่องเครื่องปลายทางเอง
  ทำครึ่ง ๆ แล้วปล่อยไว้แย่กว่าบอกตรง ๆ ว่าต้องทำต่อตาม README
