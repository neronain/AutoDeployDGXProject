...
- **บอกว่ากำลังทำอะไรอยู่ระหว่างรอ** — ขั้นตอนที่เงียบได้ตั้งแต่ 3 วินาทีถึงเป็นนาที
  (อ่าน metadata จาก Hub, เรียก LLM วางแผน, render, quality gates, ถามเครื่องอื่นผ่าน SSH)
  มีสปินเนอร์บอกชื่องานแล้ว · เคสจริง: `inspect` โมเดลใหญ่เงียบ 11 วินาทีจนมีคน Ctrl-C ทิ้ง
  เพราะคิดว่าค้าง
  · ออก **stderr** และ**เงียบสนิทเมื่อไม่ได้ต่อ terminal** — `lmds plan --json | jq`
  ต้อง pipe ต่อได้โดยไม่มีอะไรปน และบาง shell/CI รวม stderr เข้า stdout ให้เอง
  การเงียบจึงปลอดภัยกว่าการหวังว่าปลายทางจะแยกสองสายให้ถูก
