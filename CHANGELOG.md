...
เรียงใหม่ก่อนเก่า · **งานที่ยังไม่ปล่อยจะอยู่บนสุดของไฟล์นี้เสมอ**

## 0.10.0 — 2026-09-21

รอบที่เก็บงานต่อยอดที่ค้างใน ROADMAP เฟส 2 มานาน · ทำขนานกันห้าสาย แบ่งตามไฟล์ที่เป็น
เจ้าของเพื่อไม่ให้ชนกัน แล้วรวมทีละก้อนพร้อมรีวิว — ไม่ใช่ merge รวดเดียว

**สิ่งที่ได้เรียนรู้ระหว่างทาง**: โน้ตสถานะในเอกสารผิดสองที่ (wizard เครือข่ายคลัสเตอร์
และ command palette ต่างก็ "เสร็จแล้ว" ทั้งคู่ แต่ ROADMAP ยังเขียนว่าค้าง) — เสียเวลา
วางแผนงานผิดไปหนึ่งรอบก่อนจะจับได้

### เพิ่ม

- **Anthropic provider ใช้งานได้จริง** — เดิม `Settings.set_provider()` ปฏิเสธตั้งแต่ตอน
  ตั้งค่าด้วยข้อความ "อยู่ใน roadmap เฟส 2" · ตอนนี้ `lmds config set-provider anthropic`
  ใช้ได้ · Messages API พร้อม stream · ไม่ส่ง `temperature` (รุ่น 4.6+ ถอดออก ส่งไป = 400)
  และอ่านคำตอบจาก block ที่ `type=="text"` เพราะรุ่นใหม่เปิด adaptive thinking เอง
- **`lmds deploy --also-stacked`** — สร้าง bundle single + stacked ในคำสั่งเดียว คนละโฟลเดอร์
  คนละ controller คนละ API key · ใบที่สอง re-plan จากศูนย์กับ fit ของ stacked ไม่ใช่ก๊อปแผน
  เดิมมาพลิกฟิลด์ (ชื่อ flag ไม่ใช่ `--topology both` เพราะใน `deploy` topology ตั้งไม่ได้
  อยู่แล้ว — อนุมานจาก target เสมอ)
- **`lmds deploy --smoke`** — download → verify → start → test-text → stop ต่อท้าย deploy
  เป็น opt-in เพราะ deploy บน hub มักสร้าง bundle ให้ *เครื่องอื่น* (hub อาจไม่มี GPU)
  · ล้มได้ exit 6 แยกจาก exit 2 ที่แปลว่า "สคริปต์ผิดตั้งแต่ยังไม่ได้รัน"
- **i18n ไทยของคอนโซล** — เดิมหน้าเว็บอังกฤษล้วนขณะที่ CLI เป็นไทย · ปุ่มลูกโลกบนแถบหัว
  เก็บค่าแบบเดียวกับธีม/ขนาดตัวอักษร · 269 คีย์ ครอบเมนู ภาพรวม การ์ดเครื่อง/โมเดล และ
  ข้อความ error · ไม่มีคำแปล = คืนอังกฤษ ไม่ใช่หน้าพัง
- **command palette ค้นได้ทั้งฟลีต** — เดิมค้นได้แค่โมเดลกับคำสั่ง 6 ตัว ตอนนี้ค้นหน้า
  ในเมนูซ้าย ไซต์ และเครื่องทุกเครื่องรวม hub
- **preset `dgx-spark-stacked-3` / `-8`** และระบบรู้ว่ากลุ่มขนาดนี้ต้องต่อแบบไหน — ต่อสายตรง
  ถึงกันได้สูงสุด 3 เครื่อง เกินนั้นต้องมี switch · คนที่มี 4 เครื่องแต่ไม่มี switch รู้ตั้งแต่
  ตอน `lmds fit` ว่าต้องซื้ออะไร ไม่ใช่ไปเจอ "unknown topology" ตอน apply
  · **ทั้งสอง preset เป็น `tested=False`** และ `stacked-4` ไม่ถูกแตะ — `tested=True` แปลว่า
  ทีมเรารันบนเครื่องจริงแล้ว ตั้งตาม README ของคนอื่นคือลูกค้า deploy แล้วพังตอนรัน

### แก้

- **`needs-switch` เงียบมาตลอด** — รหัสเตือนนี้อยู่ใน payload ของ `cluster show` แล้ว แต่ลูป
  ที่พิมพ์ warning รู้จักแค่ `under-negotiated` และหน้าเว็บโชว์เป็นรหัสดิบ · ตอนนี้บอกว่า
  ต้องซื้ออะไร และรหัสที่ยังไม่รู้จักโผล่เป็นรหัสแทนที่จะหายเงียบ
- **`observed_cabling()` โยน KeyError แทนที่จะคืน `"unknown"`** ตามที่ docstring สัญญา เมื่อ
  สมาชิกยังไม่เคย probe — ซึ่งมีจริงในทะเบียน และฟังก์ชันนี้ถูก export ออกไปแล้ว
- ข้อความ "คลัสเตอร์ 2–4 เครื่อง" ที่ค้างอยู่ใน help ของ `lmds cluster`, `nodes/doctor.py`
  และหน้าเว็บ ทั้งที่ `infer_topology` รับถึง 8 แล้ว
- ROADMAP เขียนว่า command palette เป็นงานค้าง ทั้งที่มีอยู่ในไฟล์แล้ว

## 0.9.1 — 2026-09-21

### แก้

- **docker pull ที่ล้มรายงานสาเหตุไม่ได้** (vLLM เดี่ยวและ stacked) · `docker pull … 2> >(tee "$errlog" >&2)`
  ใช้ process substitution ซึ่ง `wait` ของ bash ไม่รับประกันว่ารอให้จบ — บรรทัดถัดไปอ่าน
  `$errlog` ตอน `tee` ยังเขียนไม่เสร็จ จึงได้ไฟล์ว่าง แล้ว controller พิมพ์ `docker บอกว่า:`
  เปล่า ๆ และหาสาเหตุไม่เจอสักข้อ ทั้งที่ docker บอกมาแล้ว · ข้อความจริงไปโผล่ท้าย stderr
  หลังทุกอย่างจบ · **คือปัญหาเดียวกับที่บล็อกนี้ถูกเขียนขึ้นมาแก้** (ลูกค้า cynbangkok 2026-09-05)
  · เปลี่ยนเป็นเขียนลงไฟล์ตรง ๆ แล้วค่อย `cat` ออกจอ
- **`mktemp` ที่ล้มทำให้ทั้งบล็อกวินิจฉัยพังเงียบ** · `$errlog` ว่าง → `2> ""` ล้มทุกครั้ง แล้ว
  `grep`/`tail` พ่น "No such file or directory" แทนสาเหตุ · ของเดิมถูกบังไว้ด้วย `tee ""`
  ซึ่งกลืน error ไปเฉย ๆ จึงไม่เคยมีใครเห็น · เลิกฝัง `/tmp` ตายตัว (เครื่องที่ `/tmp` เต็ม
  หรือ mount อ่านอย่างเดียวมีจริง) และถ้าสร้างไม่ได้จริง ๆ ก็ยังดึง image ต่อได้
  แค่บอกสาเหตุละเอียดไม่ได้

เจอเพราะเทส `test_prepare_runtime_retries_a_dropped_pull_and_names_the_real_cause`
สุ่มแดงบน CI สามครั้งในวันเดียว · สองครั้งแรก rerun แล้วเขียวจึงเข้าใจว่าเป็น flake
ครั้งที่สาม log เต็มบอกชัดว่าไม่ใช่ — เป็นบั๊กของ controller ที่เทสจับได้ถูกแล้ว

**controller ทุกตัวกลายเป็น stale** (template เปลี่ยน) — `lmds bundles refresh --all --if-older`
ออฟไลน์ ไม่แตะ `bundle.env`

## 0.9.0 — 2026-09-21

รอบที่ปิดรายการ "ต้องแก้ก่อนรีลีสเชิงพาณิชย์จริง" ทั้งห้าข้อใน `docs/RELEASE.md` §9
สามข้อเป็นเรื่องความปลอดภัยที่พังแบบ *เงียบ*: ค่าเริ่มต้นเปิดโล่ง, key ที่หายทุก reboot,
และคอนโซลที่ไม่ถามอะไรเลยบน localhost — ไม่มีข้อไหนที่ผู้ใช้จะเห็นเองจากหน้าจอ

ข้อ 2 ปิดแค่ครึ่ง: auth กับ audit เสร็จ แต่ **RBAC (บัญชีผู้ใช้รายคน) ยังไม่มี** —
เหตุผลอยู่ใน `docs/RELEASE.md` §9

### เพิ่ม

- **`lmds key` ใช้กับ bundle ที่ adopt มาได้แล้ว** · adopted controller เขียนมือใน `adopt.py`
  ไม่ได้มาจาก template จึงไม่เคยรู้จักที่เก็บ key · การถอดค่าความลับออกยังเหมือนเดิม (ไฟล์
  เป็น 0755) แต่ `--env ชื่อ` เฉย ๆ ให้ docker หยิบจาก environment ของเชลล์ที่สั่ง start ซึ่ง
  systemd ตอน autostart ไม่มีให้ → container ขึ้นมาแบบไม่มี auth ทุก reboot · ตอนนี้ controller
  ไปอ่าน `~/.lmds/keys/<slug>` เองตอน start และเติมเฉพาะตัวแปรชื่อ `*API_KEY*` (HF token /
  รหัสผ่าน เป็นคนละเรื่อง ไม่แตะ) · env จากภายนอกยังชนะเสมอ · container ที่ไม่มีตัวแปรแบบนั้น
  เลย สคริปต์เขียนไว้ตรง ๆ ว่า `lmds key` ไม่มีผลกับมัน แทนที่จะเงียบ

- **คอนโซลเว็บต้องยืนยันตัวตนเสมอ** · เดิม token ถูกสุ่มให้เฉพาะตอน bind ออก network
  ส่วน `127.0.0.1` ซึ่งเป็นค่าปริยายถูกปล่อยโล่ง (`require_token`: `if not token: return`)
  โดยถือว่า "เครื่องนี้เท่านั้น" = ปลอดภัย ซึ่งไม่จริงสองทาง: ผู้ใช้อื่นบนเครื่องเดียวกัน
  เปิด `localhost:8600` แล้วได้สิทธิ์ของ user ที่รัน hub (คุมทั้งฟลีตผ่าน SSH) และเพจใด
  ก็ได้ที่ผู้ใช้เปิดในเบราว์เซอร์ยิง POST มาที่ loopback ได้ (CSRF / DNS rebinding)
  · `--no-auth` เปิดโล่งได้แต่ต้องสั่งเองและพิมพ์คำเตือนสีแดงทุกครั้ง
- **`lmds audit` — ร่องรอยว่าใครสั่งอะไร** · `~/.lmds/audit.log` (0600) เก็บคำสั่งที่เปลี่ยน
  สถานะกับคำขอที่ถูกปฏิเสธ: เวลา · IP · method+path · ผล · ms · เก็บที่ middleware จุดเดียว
  route ใหม่จึงไม่มีทางลืม · ไม่เก็บ body และ **ไม่เก็บ query string** เพราะ token เดินทาง
  มาทาง `?token=` ได้ · `--failed` ดูเฉพาะที่ถูกปฏิเสธ (ไล่เดา token เห็นเป็นชุดจาก IP เดียว)

- **`lmds key` — API key ของ model server มีที่เก็บแล้ว** (`~/.lmds/keys/<slug>` โหมด 0600)
  เดิม key ไม่ได้ถูกเก็บไว้ที่ไหนเลย: หน้าเว็บส่ง `API_KEY=` ไปกับการกด start ครั้งเดียว
  ส่วน systemd unit เรียก `<controller> start` เปล่า ๆ — **ทุก reboot โมเดลจึงกลับมาเปิดโล่ง
  บน 0.0.0.0 เงียบ ๆ ทั้งที่ผู้ใช้ตั้ง key ไว้แล้ว** · ที่เก็บอยู่ข้าง *เครื่อง* ไม่ใช่ข้าง bundle
  เพราะโฟลเดอร์ bundle ถูก zip แจกต่อได้ · controller อ่านเองเมื่อไม่มีใครส่ง `API_KEY` มาให้
  (flag/env ยังชนะเสมอ) · `show` ไม่พิมพ์ค่าเต็มถ้าไม่สั่ง `--reveal` และ `set` รับทาง stdin
  ไม่ใช่ argv เพราะ `ps` อ่าน argv ของทุก process บนเครื่องได้
- **bundle ที่ deploy ใหม่ได้ key ตั้งแต่เกิด** — ค่าเริ่มต้นเดิมคือ bind `0.0.0.0` แบบไม่มี key
  ซึ่งเป็นค่าที่อันตรายที่สุดที่ ship อยู่ · **bundle ที่ติดตั้งไปแล้วไม่ถูกแตะ** โดยตั้งใจ:
  mint ตอน deploy ไม่ใช่ใน `render_bundle()` เพราะ `bundles refresh` ก็เรียกตัวเดียวกัน —
  แจก key ที่นั่น = client ทุกตัวของลูกค้าพังพร้อมกันหลัง update
- **`lmds doctor` ขึ้นข้อ `endpoint`** — ผูก 0.0.0.0 โดยไม่มี key = เตือน (ไม่ใช่ fail เพราะวงแลนแยก
  ที่ตั้งใจเปิดมีจริง) · controller เตือนตอน start อยู่แล้วแต่ข้อความนั้นเลื่อนหายไปกับ log ของการบูต

- **`LMDS_REPO_REF` — ปักหมุดเวอร์ชันบนเครื่องลูกค้าได้แล้ว** · ตั้งเป็น tag บน hub
  (`export LMDS_REPO_REF=v0.8.0`) แล้ว node จะได้ ref นั้นตรง ๆ ไม่ใช่ปลาย branch ปริยาย
  `LMDS_REPO_URL` เดิมเลือกได้แค่ "repo ไหน" ไม่ใช่ "commit ไหน" — ไซต์ที่มีหน้าต่าง
  เปลี่ยนแปลงหรือต้องผ่าน audit จึงใช้ไม่ได้ และเราเองก็ไม่มีทางบอกได้ว่า ณ วันเกิดปัญหา
  เครื่องนั้นรันโค้ดชุดไหน · ปักหมุดแล้ว hub จะไม่ส่ง bundle ของตัวเองไปทับ และ unit ของ
  คอนโซลเว็บถือ env ตัวนี้ไปด้วย (ไม่งั้นปุ่มในหน้าเว็บได้เวอร์ชันผิดเงียบ ๆ ทั้งที่ CLI ถูก)
- **lint gate ใน CI (ruff)** · เลือกเฉพาะกฎความถูกต้อง (F, B) ไม่เอากฎสไตล์ — เหตุผลอยู่ใน
  `pyproject.toml` · รอบแรกจับของจริงได้สองอย่าง: `running_slots` หายจาก `__all__` ของ
  `lmds.fleet` และ helper ที่ปิดทับตัวแปรลูปใน `hardware/profiler.py`

### แก้

- **adopted controller ที่มี env ความลับ `start` ไม่ขึ้นเลย** — หมายเหตุ "ค่าของ … ไม่ได้เก็บ
  ไว้ในไฟล์นี้" ถูกแทรก *กลาง* `docker run … \` · บรรทัดก่อนหน้าจบด้วย `\` ซึ่งต่อเข้า
  บรรทัดคอมเมนต์ แล้วคำสั่งจบตรงนั้น: docker ถูกยิงโดยไม่มี image และไม่มี `--env` สักตัว
  ส่วนบรรทัด `--env …` ที่เหลือกลายเป็นคำสั่งใหม่ (`--env: command not found`) · เงียบด้วย
  เพราะไฟล์ยังผ่าน `bash -n` สบาย ๆ — เทสที่ดูแต่ข้อความจึงไม่เคยจับได้ เจอตอนเขียนเทส
  ที่รันสคริปต์จริง · ย้ายหมายเหตุขึ้นไปเหนือ `docker run`
- **`lmds doctor` เลิกขึ้นเขียวให้ bundle ที่ใช้ key ที่เก็บไว้ไม่ได้** — มีไฟล์ key ไม่ได้แปลว่า
  controller หยิบไปใช้เป็น · controller ที่ render ก่อนรุ่นที่รู้จักที่เก็บ และ bundle ที่ adopt
  มาจาก container ซึ่งไม่มีตัวแปรชื่อ `*API_KEY*` ยังเสิร์ฟแบบเปิดอยู่ทั้งที่ `lmds key show`
  บอกว่ามี key — เขียวตรงนั้นคือคำโกหกที่อันตรายกว่าไม่มีการตรวจ

- **token ของคอนโซลไม่ไปอยู่บน argv อีก** · `lmds web -b` เคยส่ง `--token <ค่า>` ให้ process
  ลูกทาง argv ซึ่ง `ps` อ่านได้ทั้งเครื่อง — ขัดกับที่โปรเจกต์ระวังไว้ให้ key ของโมเดลเอง
  ตอนนี้ไปทาง environment (`/proc/<pid>/environ` อ่านได้เฉพาะเจ้าของกับ root)
- **ไฟล์ที่ถือ token ไม่มีช่วงที่อ่านได้ทั้งเครื่อง** · `web-token`, `web.json` และ unit ของ
  systemd เคยถูก `write_text()` ก่อนแล้ว `chmod(0o600)` ทีหลัง — ระหว่างสองขั้นนั้นไฟล์
  เกิดมาด้วย umask ปกติ · ตอนนี้ open ด้วย 0600 ตั้งแต่แรกแล้ว replace

- **`lmds fleet check --check` — ดูสภาพฟลีต *ตอนนี้*** · เดิม CLI ส่ง snapshot ว่างเข้า
  `fleet_report()` ทุกครั้ง จึงตกไปใช้ตัวนับจากทะเบียนเสมอ (= ของรอบที่ probe ล่าสุด)
  หน้าเว็บไม่เจอปัญหานี้เพราะมีตัว refresh เบื้องหลังคอยอุ่นแคชไว้ · เคสจริงคือ
  `node install` เสร็จแล้วแต่ `fleet check` ยังบอกเวอร์ชันเก่า คนอ่านคิดว่า install ไม่ติด
  · `--check` ต่อเข้าทุกเครื่องพร้อมกันแล้วเขียนผลกลับทะเบียนให้ด้วย เครื่องที่ต่อไม่ติด
  ถอยไปใช้ของเดิมแทนที่จะทำให้รายงานทั้งฟลีตล้ม
- **ตารางบอกอายุของข้อมูลรายแถว** (คอลัมน์ "ข้อมูลเมื่อ") และขึ้นบรรทัดเตือนเมื่อยังไม่ได้
  ต่อเข้าเครื่องจริง — ป้าย `(registry)` ตัวจาง ๆ ท้ายแถวเดิมอ่านง่ายเกินจะพลาด ·
  `last_seen` เพิ่มเข้าไปใน payload ของรายงานด้วย (หน้าเว็บและสคริปต์ใช้ต่อได้)

- **git bundle ที่ hub ส่งไปให้ node บรรจุ `HEAD` แทน `main`** · hub ที่ checkout tag หรือ
  branch อื่นไว้เคยส่ง *ปลาย main* ไปให้ทั้งฟลีต โดย stamp ยังรายงานว่า "ตรง hub" ทั้งที่
  โค้ดคนละชุด · และ hub ที่ clone มาแบบ `--branch v0.8.0` ไม่มี ref `main` ในเครื่องเลย
  ทำให้ pack ล้มแล้วถอยไปดึงจาก GitHub เงียบ ๆ ซึ่งเครื่อง air-gapped ทำไม่ได้
- `except` 50 จุดที่ raise ต่อโดยไม่บอกต้นเหตุ · CLI ใช้ `from None` (พิมพ์ข้อความให้ผู้ใช้
  แล้ว ไม่ต้องพ่วง traceback) · providers/api ใช้ `from exc`
- `zip()` 7 จุดระบุ `strict=` ชัดเจน — 5 จุดที่ความยาวการันตีเท่ากันจะดังขึ้นถ้าสมมติฐานพัง

## 0.8.0 — 2026-09-20

รอบที่ LMDS มีระบบไลเซนส์เป็นของตัวเอง — ไฟล์ลงนามออฟไลน์ล้วน ไม่มี phone-home
ไม่มี activation server ไม่มีอะไรวิ่งออกเน็ต เพราะ `SECURITY.md` สัญญาว่าไม่มี telemetry
และตลาด DGX ในองค์กรคือตลาด air-gapped

### ระบบไลเซนส์

- **เครื่องที่เสิร์ฟได้ 1 เครื่อง ใช้ฟรีทุกวัตถุประสงค์รวมเชิงพาณิชย์ โดยไม่ต้องมีไฟล์ไลเซนส์**
  ตรงกับ `LICENSE §2` ที่ประกาศไปแล้ว · control plane (ไม่มี GPU/llama-server) **ไม่นับ**
  เพิ่มได้ไม่จำกัด · ตั้งแต่ 2 เครื่องที่บริหารร่วมกันขึ้นไปจึงต้องมีใบ
- **นับตาม `LICENSE §1.1` ไม่ใช่นับแถวในทะเบียน** — ใช้ `ServingCapability` ตัวเดียวกับที่
  `doctor` ใช้อยู่แล้ว · เครื่องที่ยังไม่เคย probe **ไม่นับ** และแสดงให้เห็นว่านับไม่ครบ
  (ผิดพลาดไปทางไม่บล็อกเสมอ)
- **จุดบังคับใช้มีที่เดียว** `licensing/enforce.py` — มีเทสบังคับว่าห้ามมี `require()` ที่อื่น
  ลูกค้าองค์กรที่ต้อง audit โค้ดก่อนซื้ออ่านไฟล์เดียวจบ
- **กฎที่เทสคุมไว้** — ห้ามล็อก `doctor`/`repair`/`validate`/quality gates · **หมดอายุ =
  อ่านอย่างเดียว ไม่ใช่ดับเครื่อง** (`ps` `logs` `start` `stop` ยังทำได้ทั้งหมด) ·
  ล็อก "ขนาด" ไม่ล็อก "ความสามารถ"
- `lmds license show` · `seats` · `install` และแผงใน **Hub settings → License**
- ตรวจลายเซ็นด้วย `cryptography` ถ้าเครื่องนั้นมี ไม่มีก็ใช้ตัวในแพ็กเกจ (RFC 8032)
  เพื่อให้เครื่อง air-gapped ที่ลง wheel ที่ต้องคอมไพล์ไม่ได้ยังใช้ได้ · ทั้งสองทางถูกเทส
  ด้วย test vector ของ RFC 8032 ชุดเดียวกัน **และมีเทสเทียบสองทางว่าตอบเหมือนกันทุกอินพุต**

  > บั๊กที่เทสนั้นจับได้: `verify(ศูนย์ 32 ไบต์, "x", ศูนย์ 64 ไบต์)` ให้ **True** บน
  > cryptography/OpenSSL (ตรวจแบบ cofactorless ไม่ปฏิเสธจุด small-order) แต่ **False**
  > บนตัวในแพ็กเกจ — เครื่องสองเครื่องในฟลีตเดียวกันจะตัดสินไฟล์เดียวกันไม่เหมือนกัน
  > ขึ้นกับว่าเครื่องไหนบังเอิญมี cryptography ลงไว้ · ย้ายด่านตรวจรูปแบบ (ความยาว ·
  > จุด small-order · `s` ที่ reduce แล้ว) มาไว้ **ก่อน** แยกทาง ทั้งสองทางจึงเข้มเท่ากัน

### ตราประทับบน bundle

- `MODEL_PROFILE.yaml` ได้ก้อน `origin:` และหัว controller ได้บรรทัด `Origin:` บอกว่า
  bundle นี้ generate บนเครื่องที่ถือไลเซนส์ใบไหน · เพิ่ม gate `origin-stamp`
- **ไม่ใช่การล็อก** — เครื่องโหมดฟรีประทับตรา `community` ได้ตามปกติ และ bundle เก่า
  ที่ไม่มีตราก็ผ่านด่าน · ด่านจับอย่างเดียวคือ *ตราที่ถูกแก้*
- ตราพก **ลายเซ็นของไลเซนส์ใบนั้นทั้งดวง** ไปด้วย ผู้รับ bundle จึงตรวจที่มาได้เอง
  โดยไม่ต้องมีไฟล์ไลเซนส์ของเรา · แก้ `licensed_to` แล้วลายเซ็นไม่ตรงกับ payload ทันที
- ข้อจำกัดที่ต้องรู้: payload ที่ติดไปมี `contact` กับ `note` ด้วย (ต้องครบถึงจะตรวจได้)
  — ออกใบโดยไม่ใส่สองฟิลด์นั้นได้ ดู `docs/LICENSING.md`

### บั๊กที่เจอจากการใช้งานจริง

- **`doctor` ฟ้อง `.download.lock` ว่าเป็น weight เสีย** — เป็น flock ที่ controller สร้าง
  ด้วย `exec 9>` จึงขนาด 0 เสมอโดยธรรมชาติ · กลายเป็นวงวนบนเครื่องลูกค้า: ฟ้องว่าเสีย →
  แนะ `repair` → repair โหลด weight 21 GB ใหม่ **และหยุดโมเดลที่กำลังเสิร์ฟอยู่** →
  จบแล้วทิ้ง lock ไว้อีก → ฟ้องอีก · weight มี `.sha256-ok` ครบตลอดทาง ไม่เคยเสียเลย
- **bundle ที่ตั้งชื่อเอง regenerate controller ออฟไลน์ไม่ได้ตลอดกาล** — ด่านตัดสินจาก
  *รูปแบบของชื่อ* (ต้องเท่ากับหรือขึ้นต้นด้วย slug ของโมเดล) ชื่อสั้นที่ผู้ใช้ตั้งเองจึงถูก
  ปฏิเสธทุกครั้ง เครื่องนั้นขึ้น "controller ค้าง" ค้างถาวรไปถึง "ตรง hub" ไม่ได้อีก ·
  และ `lmds rebuild` ไม่ใช่ทางออกเพราะสร้าง bundle ก้อนที่สอง = โมเดลซ้ำสองตัวบนเครื่องเดียว ·
  ตัวตัดสินจริงคือ `MODEL_PROFILE.yaml` ในโฟลเดอร์นั้น ไม่ใช่ชื่อ

### หน้าเว็บ

- **เมนูซ้ายลากขยายได้** (190–460px) ดับเบิลคลิกกลับค่าเดิม ลูกศรซ้าย/ขวาก็ได้ ·
  จำค่าไว้และใส่ตั้งแต่ก่อนวาดจึงไม่กระพริบ · 238px ตายตัวทำให้ชื่อเครื่องถูกตัดเกือบทุกแถว
  เมื่อฟลีตโตขึ้น
- **แถบเลื่อนของเมนูเลิกเด่นกว่าตัวเมนู** — ของ default บนพื้นมืดกลายเป็นเส้นขาวหนา
- **ตัวเลขท้ายแถวในเมนูอยู่คอลัมน์เดียวกันทุกแถว** — เดิมยืนสองคอลัมน์ห่างกัน 9px
  เพราะแถวที่มีลูกศรพับกับแถวที่ไม่มีเรียงไม่เหมือนกัน

### หมายเหตุสำหรับคนอัปเดต

template เปลี่ยน → `template_hash` เปลี่ยน → controller เดิมทั้งฟลีตกลายเป็น stale และจะถูก
regenerate แบบออฟไลน์ตอน `lmds node install` ซึ่งเป็นกลไกปกติอยู่แล้ว · **contract ไม่ได้
เปลี่ยน** `TEMPLATE_STANDARD` จึงยังเป็น `dgx-spark-controllers-v3.0.0`

---

## 0.7.0 — 2026-09-20

รอบนี้ทุกข้อมาจากการเอาไปใช้จริงบนเครื่องใหม่ (3× RTX 3060 + DGX Spark ที่ไซต์ลูกค้า)
ไม่ใช่การไล่อ่านโค้ดหาของเสีย

### ตัวเลขบนจอที่ไม่ตรงกับความจริง

- **VRAM ของเครื่องหลายการ์ดขึ้นแค่ใบเดียว** — 3× RTX 3060 12 GB โชว์ "12 GB" ขณะที่
  `lmds fit` บนเครื่องเดียวกันคำนวณ 36 GB · 6 จุดในหน้าเว็บอ่าน `gpus[0]` · `vram_used_gb`
  รวมเฉพาะเมื่อทุกใบรายงาน (GB10 ไม่รายงานเลย) · อุณหภูมิ/utilization ใช้ค่าสูงสุดไม่ใช่ผลรวม
- **container ที่ตายไปสองสัปดาห์รายงานว่ากำลังรัน** — `docker ps --filter name=^X$` เป็น
  regexp ของ Go ไม่ใช่การเทียบสตริง และ slug เต็มไปด้วยจุด ชื่อของตัวที่ตายจึงไปแมตช์
  container คนละตัวที่ยังรันอยู่แล้วยืมสถานะมา · เปลี่ยนไปถาม
  `docker container inspect '{{.State.Running}}'`
- **fit ทำนายต่ำไป 4.65 GB บนเครื่อง 3 ใบ** — overhead เป็นค่าต่อการ์ดแต่ถูกหักครั้งเดียว
  (`_budget_gb` คูณ `gpu_count` มาตลอด `plan_kv_pin` ไม่คูณ) และ mmproj ไม่ถูกนับใน weights ·
  ส่วนต่างเคยโผล่เป็นแถว "โปรเซสอื่น 4.6 GB" ที่ไม่มีอยู่จริง · ไม่มีค่าคงที่ตัวไหนเปลี่ยนค่า
  หน่วยของมันผิด — Spark มี 1 GPU/เครื่อง "ต่อโมเดล" กับ "ต่อใบ" จึงแยกไม่ออกที่นั่น

### stacked

- **หา GID index ของ RoCE v2 เองต่อเครื่องต่อ HCA** แทนเลข 3 ที่ฝังไว้ ซึ่งดริฟท์หลังรีบูต
  แล้ว NCCL init ค้างนิ่ง ๆ โดยไม่มีข้อความ · `docs/DGX-SPARK-VLLM-FIELD-NOTES.md:111` สั่งไว้
  ตั้งแต่แรกแต่โค้ดไม่เคยทำ · ไม่เจอแล้วดัง ไม่ตกกลับไปที่ 3
- **hub ตั้งไซต์ของตัวเองได้** — `nodes/cluster.py` จับกลุ่ม stacked ด้วย `site` และแถวของ hub
  hardcode `""` ไว้ hub จึงจับคู่กับ node ในไซต์เดียวกันไม่ได้เลย แม้ฮาร์ดแวร์ตรงกันทุกอย่าง

### healthcheck

- **`--no-healthcheck` ทั้งสี่ template** — image ของ llama.cpp ฝัง `curl localhost:8080` ไว้
  ตายตัว LMDS เลือกพอร์ตเอง container จึงขึ้น `(unhealthy)` ตลอดทั้งที่ `/health` ตอบ 200 ·
  สำหรับ SGLang อันตรายจริงเพราะ `/health` รัน prefill ทุกครั้ง · รวม DRY RUN array และ
  container downloader ที่ไม่มี API เลย

### เอกสารและสูตร

- **ถอนเพดาน "switch รองรับ 4 เครื่อง"** ที่ไม่มีที่มา — ตอนนี้มีคลัสเตอร์ 8 เครื่องเสิร์ฟอยู่จริง
  สองชุด · กฎ "≤3 ต่อสายตรง" ถูกต้อง เก็บไว้พร้อมเหตุผลจริง
- **เพิ่มหัวข้อ GPU clock latch** ที่เอกสารไม่เคยมี — EC ล็อก GPU ไว้ 631-949 MHz โดย
  `nvidia-smi` ไม่แสดงอะไรผิด แก้ได้ทางเดียวคือถอดปลั๊ก 30-60 วินาที
- **สูตร "ห้าม deploy" ตัวแรก** — `nvidia/DeepSeek-V4.1-Flash-NVFP4` 14 boot บน 4 เครื่อง
  ไม่เคยเสิร์ฟได้ · เหตุอยู่ที่ loader ของ vLLM ไม่ใช่การตั้งค่า
- **gpu-util ผูกกับจำนวน rank** — 0.80 ที่ TP4 / 0.75 ที่ TP8
- **อย่าเชื่อเลข "KV pool tokens" ที่ engine พิมพ์** — เป็น `max_concurrency × max_model_len`
- `docs/RELEASE.md` · `docs/HCA-DUAL-TEST.md` · `THIRD-PARTY-LICENSES` ใหม่

### License

- **source-available แทน proprietary** — ข้อความเดิมเขียนว่า "No license is granted to **use**"
  อ่านตามตัวอักษรแล้วไม่มีใครมีสิทธิ์รัน LMDS เลย · ตอนนี้ **1 เครื่อง = ฟรีทุกวัตถุประสงค์
  รวมเชิงพาณิชย์ · ตั้งแต่ 2 เครื่องที่บริหารร่วมกัน = ต้องมี license** · ใช้ย้อนหลังทุกเวอร์ชัน
...
- **ชื่อ bundle จาก repo ยาว ๆ ไม่ทำให้ push ไม่ได้อีก + `lmds deploy --name`** — 2026-09-08 hub aicontrol: deploy repo
  ...
  จนดูเหมือนชื่อลงท้ายด้วยขีด) · `--name` ตั้งชื่อเองได้ · เทส `test_generator.py`
...
ฟลีต 15 เครื่อง / 47 bundle ต้องเลื่อนหลายจอกว่าจะเจอเครื่องที่ต้องการ · ผู้ใช้ 2026-09-04 ขอแบบแดชบอร์ด:
"เมนูอยู่ซ้าย กดรายการแล้วรายละเอียดออกตรงกลาง site/node อยู่ซ้าย กดแล้วรายละเอียดแต่ละ node แสดงตรงกลาง"
— ออกแบบเป็น mockup คลิกได้ด้วยข้อมูลจริงก่อน ผ่านแล้วค่อยทำ

- **App shell**: แถบซ้าย 238px = ภาพรวม · เครื่องนี้ (hub) · ทุกเครื่อง · ต้นไม้ **ไซต์ → เครื่อง** (จุดสถานะ +
  แถบ % หน่วยความจำ มองปุ๊บรู้ว่าใครแน่น) · คลัง (โมเดลทั้งฟลีต · คะแนน · สูตร · weights · ตั้งค่า) ·
  ด้านบน = breadcrumb · ค้นเครื่อง/โมเดล (Enter แล้วกระโดดไป) · Deploy model
- **Router แบบ hash** (`#/overview` `#/node/<ชื่อ>` `#/site/<ไซต์>` `#/models` …) — ลิงก์ตรงถึงเครื่องได้ ·
  กด back/forward ได้ · reload แล้วอยู่หน้าเดิม
- **ไม่รื้อ logic**: การ์ดเครื่องยังอยู่ใน `#nodes` เหมือนเดิม (nodeRows / handler / SSE / drag-จัดลำดับ /
  รั้ว cluster ไม่รู้ด้วยซ้ำว่ามี shell) · router แค่เลือก view และในหน้าเครื่อง**ซ่อน**การ์ดของเครื่องอื่นด้วย
  `hidden` — เทสหน้าเว็บ 500+ ข้อผ่านโดยไม่แก้ · id ทุกตัวเดิมครบ
- **หน้าภาพรวม** (คำนวณจากแคชเดียวกับการ์ด ไม่ยิงเพิ่ม): KPI เดิม + แถบหน่วยความจำ GPU ต่อเครื่องเรียงจาก
  แน่นสุด (คลิกชื่อ → หน้าเครื่อง) · โดนัท llama.cpp/vLLM ที่รันอยู่ · **"ต้องดู"** จากข้อมูลจริง —
  เครื่องต่อไม่ได้ · เครื่องเกิน 90% · พอร์ตซ้อนกันบนเครื่องเดียว · commit ไม่ตรง hub · ตารางไซต์
- **หน้าโมเดลทั้งฟลีต**: ตารางเดียวทุก bundle ทุกเครื่อง คลิกแถว → หน้าเครื่อง + เปิดแผงของโมเดลนั้น
- **หน้าเครื่อง**: การ์ดเดิมของเครื่องนั้นกางเต็ม (เกจ · GPU · โมเดล · แผง settings + บรรทัดคำนวณแรม) ·
...
**ฟอร์ม settings บอกว่า context / slots / gpu-util ที่กรอกต้องใช้แรมเท่าไร — ก่อนกด start**

ผู้ใช้ 2026-09-04: "หลังกรอกแล้ว จะมีให้ดูว่าต้องการแรมเท่าไร เพราะถ้าใส่ gpu-util น้อยกว่าที่ต้องการ ระบบจะ
ทำงานไม่ดีไหม" · ใช่ และแย่กว่านั้น: vLLM จอง gpu-util × แรมทั้งเครื่อง เอา weights ออก ที่เหลือคือ KV ·
เหลือไม่พอ 1 คำขอที่ context ที่ตั้ง = **ไม่ start เลย** · จองมากกว่าที่ว่างอยู่ (เครื่องที่รันตัวอื่น) = ไม่ start
เหมือนกัน · เดิมรู้ตอนกด start แล้วพังเท่านั้น (เคสในภาพผู้ใช้: context 131077 · slots 3 · gpu-util 0.85
บน veerasiam ที่ว่าง 27 GB)

- บรรทัดใต้แถว port/context/slots/gpu-util คำนวณสดทุกครั้งที่พิมพ์: `จอง X GB → weights → เหลือให้ KV Z GB ·
  KV/คำขอที่ context นี้ · รับได้ N คำขอเต็ม context (slots ที่ตั้ง S)` แล้วฟันธง ✅/⚠️/❌ พร้อมทางแก้เป็นตัวเลข
  ("ตั้ง gpu-util ≥ 0.31 หรือลด context เหลือ ≤ 131,072" / "ตั้ง gpu-util ≤ 0.20 หรือหยุดโมเดลอื่นก่อน")
- llama.cpp คิดคนละแบบ: จอง KV ทั้งก้อนตาม --ctx-size ล่วงหน้า และแบ่ง context ให้ทุก slot — บรรทัดบอกทั้งสองอย่าง
- `GET /api/nodes/{name}/models/{slug}/memory` (+ `/api/models/{slug}/memory`) ส่งแค่ข้อเท็จจริง: weights,
  KV/token, ความจุ, **ว่างตอนนี้** (unified = แรมว่างของทั้งระบบ ไม่ใช่ความจุ − ของที่โมเดลถือ), overhead ·
  เลขคณิตอยู่ในเบราว์เซอร์จึงขยับตามทุกตัวอักษรโดยไม่ยิง request
- MODEL_PROFILE.yaml ของ bundle ใหม่มี `kv_bytes_per_token` · bundle เก่า hub ถาม Hugging Face ครั้งเดียวแล้วจำ ·
  ถามไม่ได้ก็ยังบอก weights และเหตุผลที่ KV หาย
- จัดแถวฟอร์มใหม่: port · context · slots · gpu-util อยู่แถวเดียวกว้างคงที่ (เดิม gpu-util ตกไปแถวล่างเดี่ยว ๆ) ·
  bind แยกแถว · เตือน context ที่ไม่ใช่ขั้นมาตรฐาน (เช่น 131077)
...
## 0.3.1–0.3.5 — รายการที่ค้างอยู่ใต้หัวข้อ `[Unreleased]`

> **หมายเหตุบรรณาธิการ (2026-09-20)** — หัวข้อนี้เคยเป็น `## [Unreleased]` ค้างอยู่ตรงนี้ตั้งแต่ยุค 0.3.x
> ทุกข้อในนี้ **ปล่อยออกไปแล้ว** ก่อน 0.3.6 (เช็คได้: `lmds node set --site` และ
> `lmds recipes --publish` อยู่ใน CLI และในคู่มือปัจจุบัน) · มันไม่ใช่งานค้าง และตำแหน่งของมัน
> ในไฟล์ก็ถูกแล้วตามลำดับเวลา (อยู่ระหว่าง 0.3.5 กับ 0.3.0) · เปลี่ยนหัวข้อให้ตรงความจริง
> เพราะ "Unreleased" กลางไฟล์ประวัติทำให้คนอ่านคิดว่ามีของค้าง
>
> **งานที่ยังไม่ปล่อยอยู่ที่หัวข้อบนสุดของไฟล์เสมอ** ไม่ใช่ตรงนี้ ·
> แผนรอบถัดไปอยู่ที่ `docs/UPGRADE-2026-09.md`
>
> หัวข้อยุคนี้ใช้สไตล์ `## [x.y.z] — วันที่` ส่วนตั้งแต่ 0.3.5 ขึ้นไปใช้ `## x.y.z — วันที่` (ไม่มีวงเล็บเหลี่ยม)
