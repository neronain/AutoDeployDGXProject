...
- **`lmds up <ลิงก์>` — ลิงก์เดียวจบถึงเซิร์ฟเวอร์ตอบได้จริง** · `lmds deploy` หยุดที่ bundle
  ...
  · เดินให้ทั้งชุดพร้อมเลขขั้นตอน แสดง output สดทุกขั้น (progress ของ download, log ตอนโหลดโมเดล)
  · ล้มขั้นไหนหยุดตรงนั้น คืน exit code ของ controller ตรง ๆ และบอกว่าดูต่อที่ `lmds doctor` /
  `lmds logs` / รันซ้ำเฉพาะขั้นนั้นยังไง
  · จบแล้วบอกวิธีต่อ client ด้วย `client-config` ต่อทันที
  · **stacked ไม่ทำแทนและคืน exit 1** — ขั้น sync-worker/verify-worker ต้องตัดสินใจเรื่องเครื่องปลายทางเอง
  (bundle ยังถูกสร้างไว้ให้) เพื่อไม่รายงาน success ทั้งที่เซิร์ฟเวอร์ยังไม่พร้อม
