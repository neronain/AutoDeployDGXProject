...
```bash
lmds license show        # สถานะ + ตัวเลขที่นับได้ (ไม่ต่อเน็ต ไม่ส่งอะไรออกไป)
lmds license seats       # นับใครบ้างและทำไม — control plane ไม่นับตามสัญญา
lmds license install <ไฟล์.yaml>
```

เครื่องที่ **เสิร์ฟโมเดลได้** 1 เครื่อง (บวก control plane กี่เครื่องก็ได้) ใช้ฟรีทุกวัตถุประสงค์
รวมเชิงพาณิชย์ **โดยไม่ต้องมีไฟล์ไลเซนส์เลย** · ตั้งแต่ 2 เครื่องที่บริหารร่วมกันขึ้นไปจึงต้องมีใบ

ไลเซนส์เป็นไฟล์ลงนามออฟไลน์ล้วน — ไม่มี activation server ไม่มีอะไรวิ่งออกเน็ต · ใบหมดอายุ
**ไม่หยุดของที่รันอยู่** แค่ขยายฟลีตเพิ่มไม่ได้ · รายละเอียดครบใน [LICENSING.md](LICENSING.md)
...
| **Hub settings** | Planning LLM · **Site of this machine** · **License** — สถานะไลเซนส์ของ hub นี้ (ผู้ถือ · ระดับ · เครื่องที่ครอบคลุม · วันหมดอายุ · จำนวนที่นับได้จริง) และช่องวางไฟล์ไลเซนส์ใหม่ ซึ่งตรวจลายเซ็นก่อนเขียนเสมอ — ใบที่ใช้ไม่ได้จะไม่เขียนทับใบเดิม (เท่ากับ `lmds license install`) |
...
quality gates ทั้ง 13 ด่านตรวจได้แค่ว่า **สคริปต์ถูกต้อง** ไม่ได้บอกว่ารันแล้วได้คำตอบจริง
...
| stacked `start` รอ /health นานมาก หน่วยความจำ head ยังต่ำ (ยังไม่โหลด weight) แล้วล้ม | head กับ worker จับมือกัน (torch.distributed/NCCL) ไม่ติด — ไฟร์วอลล์ · IP/interface สายเร็วผิด · RoCE/GID | ตั้งแต่ 0.6.0 controller เตือนเองหลัง 4 นาทีพร้อมผล ping สองทาง/พอร์ต master/ท้าย log ทั้งสองฝั่ง · เช็ค `sudo ufw status` ทั้งสองเครื่อง (ปล่อย MASTER_PORT 25000 + ephemeral บนสายเร็ว) · `<controller> network-info` · ลอง `NCCL_IB_DISABLE=1 <controller> restart` · `lmds set <slug> --engine-env NCCL_DEBUG=INFO` |
| llama.cpp ตั้ง context 131,072 แต่ Score/`/props` บอก 65,536 | llama.cpp แบ่ง `--ctx-size` ให้ทุก slot (`--parallel`) เท่ากัน — ค่าที่ตั้งคือก้อนรวม ไม่ใช่ต่อ request (ต่างจาก vLLM) | การ์ดโชว์ป้าย `N/request` · ต้องการ 128K ต่อ request กับ 2 slots → ตั้ง context 262144 (KV ใหญ่ขึ้น 2 เท่า) หรือ slots 1 |
| `download` ขึ้น "กำลังรันอยู่แล้ว (อีก process ถือ .download.lock)" | มี process อื่นถือ flock อยู่จริง (ตัวแม่ตายแต่ curl ยังโหลด หรือสั่งซ้อนจาก hub+CLI) | ข้อความบอก pid/คำสั่งที่ถืออยู่แล้ว (ใช้ fuser/lsof) — รอให้จบแล้วสั่งซ้ำ ไฟล์ต่อจากเดิม หรือ `kill <pid>` ตัวนั้น |
| clone แล้วดิสก์ปลายทางเต็มกลางทาง | เดิมไม่เช็คเนื้อที่ก่อน | ตั้งแต่ 0.6.1 `lmds node clone` ถามเนื้อที่ปลายทางก่อน (เผื่อ 5%) และปฏิเสธพร้อมบอกว่าเหลือเท่าไร |
| GLM-5.3-Flash (glm5_next) ตายตอน warm-up `concat_and_cache_mla … pe_dim must be 64 for fp8_ds_mla` | บน DGX Spark (SM121) vLLM stock/nightly และ image glm53-flash ของ Red Hat มี sparse-MLA backend ตัวเดียวที่บังคับ KV layout ของ DeepSeek แต่ GLM ใช้ MLA แบบ NoPE | ใช้สูตรใน catalog (image ชุมชน `ghcr.io/tonyd2wild/vllm-glm53-flash` sm121-v8 · kv fp8_e4m3 · marlin · block 2304 · `--kv-cache-memory 3758096384` (pin KV 3.5 GiB) · CUDA graph · ctx ≤ 262144 · parser glm47/glm45) — deploy ใหม่จะได้เอง · bundle เก่า: `lmds set <slug> --image ghcr.io/tonyd2wild/vllm-glm53-flash:sm121-v8 --context 262144 --extra-args "--kv-cache-dtype fp8_e4m3 --moe-backend marlin --block-size 2304 --trust-remote-code --kv-cache-memory 3758096384"` แล้ว prepare-runtime + start · ห้ามตั้ง context เกิน 262144 (ชน `persistent_topk oversubscribe`) · RAM: ปล่อย gpu-util 0.85 = เครื่อง 115/121 GiB + swap 4 GB → pin KV แล้ว context ไม่มีผลกับ RAM อีก · MTP เปิดไม่ได้กับ orcarouter/coolbho3k (checkpoint ไม่มีเลเยอร์ MTP) มีเฉพาะ RedHatAI |
| worker log: `DistNetworkError: The client socket has timed out … trying to connect to (<head-ip>, 25000)` / head: `waiting for clients. 1/2 clients joined` | ufw บน head กัน TCPStore/NCCL ของ worker (ping ถึงแต่ TCP ไม่ถึง) | `sudo ufw allow in on <iface สายเร็ว>` บน head (0.6.1: `lmds cluster apply` ทำให้ · inspect เตือน) · ยืนยัน: `ssh <worker> 'nc -zv -w3 <head-ip> 25000'` · แล้ว restart *โดยไม่ใส่* `NCCL_IB_DISABLE=1` เพื่อให้ RoCE ทำงาน |
