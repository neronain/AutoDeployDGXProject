...
คำสั่งในเมนู:
`restart` · `doctor` · `logs` (300 บรรทัดล่าสุด) · `repair` · `enable`/`disable` (autostart)
— ปุ่มขึ้นตามสถานะจริงของโมเดลนั้น (ยังไม่ได้รันจะไม่มี `restart` ให้กด)

**`remove` จงใจไม่มีในเมนู** — มันลบ weight หลายสิบ GB บนเครื่องปลายทาง และต้องใช้ `-y`
ซึ่งข้ามหน้ายืนยันที่แสดงรายการ+ขนาดไฟล์ · ลบจากเครื่องที่คุณนั่งอยู่ได้ด้วย
`lmds node run <ชื่อเครื่อง> remove <ชื่อโมเดล>` ซึ่งยังถามยืนยันตามปกติ
· allowlist บังคับที่ฝั่ง server ไม่ใช่แค่ซ่อนปุ่ม
