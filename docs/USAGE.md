...
RoCE local ขึ้นครบสี่ตัวบอกได้เพียงว่าเครื่องนั้น **อาจต่อสองพอร์ต**. LMDS จะแสดงเป็น
`mesh candidate` และเตือนเรื่อง OOB candidate แต่ไม่อ้างว่าปลายสายเป็น direct mesh จริง — ต้องตรวจ
route/การมองเห็นกันจากทุก node. Controller ไม่ฉีดค่า NCCL เฉพาะของโครงการอื่นอัตโนมัติ.

LMDS ยังไม่ตรวจ MTU และ branch นี้ไม่มี benchmark ที่ทำซ้ำได้เพียงพอจะรับรองว่า MTU ใดเหมาะกับ
ทุก driver/firmware/topology; ให้ทดสอบ end-to-end ตามคู่มือของ fabric ที่ใช้งานจริง.
