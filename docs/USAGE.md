...
#### คำเตือนเรื่องการเดินสายที่ขึ้นให้เอง

ConnectX ของ DGX Spark: **หนึ่งพอร์ต QSFP = RoCE คู่แฝดสองตัว** (SoC จ่าย PCIe 5.0 ได้แค่
x4 ต่อ device) ต่อสายเส้นเดียวจึงมีสอง device ที่ต้องบอก NCCL ให้ครบ

`lmds node cluster` และ `lmds hardware` เตือนสามอย่างนี้ — ทุกข้อคือเคสที่ **"ก็ยังรันได้"**:

| ⚠ | หมายความว่า | แก้ยังไง |
|---|---|---|
| ลิงก์ขึ้นแต่ยังไม่มี IP | สายเสียบอยู่แต่ NCCL ใช้ไม่ได้ = ได้แบนด์วิดท์ครึ่งเดียว | ตั้ง IP ให้เส้นนั้น (netplan) |
| คู่แฝดอยู่วง subnet เดียวกัน | routing สับสน แพ็กเก็ตออกผิดเส้น | แยกคนละ `/24` |
| ไม่มี RoCE device คู่กัน | ตั้ง `NCCL_IB_HCA` ไม่ได้ → ตกไปใช้ TCP | ตรวจ driver/`ibdev2netdev` |

ต่อ **สองพอร์ต** ต่อเครื่อง = RoCE ขึ้นครบสี่ = ผัง **mesh 3 เครื่องโดยไม่ต้องมีสวิตช์** ·
controller ตรวจเองแล้วใส่ค่า NCCL เฉพาะของ mesh ให้ตอน `start` และระบบจะเตือนถ้ายังไม่มีสาย
out-of-band (RJ-45 10G) ที่ทุกเครื่องเห็นกัน — ซึ่ง NCCL/Ray ต้องใช้ตอน bootstrap

> **MTU ไม่ต้องตั้ง** — คู่มือทั่วไปสั่ง `mtu 9000` แต่วัดบน DGX Spark สองเครื่องจริงแล้ว
> ได้เท่ากันเป๊ะ (111.71 Gb/s · 1.98 µs ทั้งที่ 1500 และ 9000) เพราะคอขวดคือ PCIe 5.0 x4
> ต่อ RoCE device ไม่ใช่ขนาดเฟรม · LMDS จึงไม่เตือนเรื่องนี้
...
**ลิงก์อยู่ยาว — bookmark ได้** token ถูกจำไว้ที่ `~/.config/lmds/web-token` (0600) และใช้ซ้ำ
ทุกครั้งที่เปิด · restart/stop แล้วเปิดใหม่ก็ยังเป็นลิงก์เดิม เปลี่ยนเมื่อสั่ง `--new-token`
หรือตั้งเองด้วย `--token` เท่านั้น
· เดิมสุ่มใหม่ทุกครั้ง ทำให้ต้องกลับไปหา terminal ทุกรอบ ซึ่งขัดกับเหตุผลที่มีหน้าเว็บตั้งแต่แรก
