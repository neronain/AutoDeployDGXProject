...
เครื่องที่ยังไม่มี `lmds` จะขึ้นว่า **Unreachable** ตลอด เพราะ hub ไม่ได้ส่ง agent ไปรัน
แต่เรียก `lmds agent info` บนเครื่องนั้น — เดิมต้อง ssh เข้าไปติดตั้งเอง ซึ่งขัดกับเหตุผล
ที่มีหน้าเว็บตั้งแต่แรก

กดปุ่ม **install** ที่หัวแถวของเครื่องนั้น (หรือปุ่มในกล่อง Unreachable) — clone repo แล้วรัน
`install.sh` ผ่าน SSH ให้ ใช้เวลาหลายนาที ระหว่างนั้นหน้าเว็บไม่ค้าง และงานเดียวกันซ้อนกัน
บนเครื่องเดียวไม่ได้ (`git pull` ซ้อน `git pull` = repo พัง)

> **ปุ่ม install ในหน้าเว็บข้ามขั้น Docker/NVIDIA toolkit โดยตั้งใจ** — ขั้นนั้นต้องใช้ `sudo`
> ซึ่งหน้าเว็บไม่มี tty ให้กรอกรหัสผ่าน เครื่องที่ยังไม่มี Docker ต้องไปรัน `./install.sh`
> บนเครื่องนั้นเอง

### ที่อยู่สำรองของเครื่อง

ปุ่ม **addresses** — ใส่ที่อยู่อื่นคั่นจุลภาค · hub ลองที่อยู่หลักก่อน แล้วค่อยไล่ตัวสำรอง
ใช้ตอนเครื่องเดียวกันเข้าได้ทั้งทาง LAN ที่ออฟฟิศและทาง VPN จากข้างนอก (เท่ากับ
`lmds node set <ชื่อ> --alt-host`)

### ยังต้องใช้ CLI สำหรับ

`lmds config` (ตั้ง provider / API key) · `lmds hardware` · และทุกขั้นที่ `sudo` ต้องกรอกรหัส
(หน้าเว็บไม่มี tty — ถ้าเปิด/ปิด autostart ไม่สำเร็จ เว็บจะบอกคำสั่งให้ไปรันเอง)
