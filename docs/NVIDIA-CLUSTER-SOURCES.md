...
> - **2026-09-21** — ย้ายกฎ "ต่อกันอย่างไร" เข้าโค้ด (ตารางท้ายไฟล์ข้อ 2) · **ไม่มีหลักฐานใหม่
>   เพิ่มเข้ามา** ระดับหลักฐานของทุกบรรทัดด้านล่างยังเท่าเดิม
...
**แก้ 2026-09-21 — `fit/targets.py` มีครบแล้ว**: `dgx-spark-single` (1) · `dgx-spark-stacked` (2) ·
`dgx-spark-stacked-3` (3) · `dgx-spark-stacked-4` (4) · `dgx-spark-stacked-8` (8) ·
ทุกตัวตั้งแต่ 3 เครื่องขึ้นไปเป็น **`tested=False`** เพราะทีมเราไม่เคยรันเอง —
`tested=True` ในโค้ดนั้นแปลว่า "ทีมเรารันบนเครื่องจริงแล้ว" และปิดโหมด conservative ของ budget

และมีแนวคิดว่า*ต่อกันอย่างไร*แล้ว — `interconnect_for(n)` ใน `fit/targets.py` เป็นที่เดียวที่เก็บ
เพดาน `MAX_DIRECT_NODES = 3` / `MAX_SWITCH_NODES = 8` · `nodes/netplan.py` (เดาผังจากสายจริง)
และ `nodes/cluster.py` (กลุ่มที่จับได้) อ่านเลขจากที่เดียวกันนี้ ไม่เขียนซ้ำ
...
> รายการนี้อยู่ในโค้ดแล้ว (`SWITCH_SHOPPING_LIST` / `DIRECT_SHOPPING_LIST` ใน `fit/targets.py`
> และ `SWITCH_SHOPPING_HINT` ใน `nodes/netplan.py`) — แก้ที่นี่แล้วต้องแก้ที่นั่นด้วย
...
| 2 | บันทึกเพดานการต่อสาย (ตรง ≤3) | **ทำแล้ว 2026-09-21** — `interconnect_for()` ใน `fit/targets.py` เป็นที่เดียวที่เก็บเพดาน · เพิ่ม preset `dgx-spark-stacked-3` และ `-8` (ทั้งคู่ `tested=False`) · `lmds fit` เตือนตั้งแต่ตอนเลือก target ว่าเกิน 3 เครื่องต้องมี switch รุ่นไหน · `lmds cluster inspect/plan` ปฏิเสธผังวงแหวน 4 เครื่องพร้อมรายการของที่ต้องซื้อ · `cluster_groups()` แนบ `interconnect` + เตือน `needs-switch` |
| 2b | ให้ CLI/หน้าเว็บอ่านรหัสใหม่ | **ยังไม่ทำ** — `cluster_groups()` ส่ง warning `needs-switch` และฟิลด์ `interconnect` ออกมาแล้ว แต่ `cli/main.py` วน warning เฉพาะ `under-negotiated` (รหัสอื่นเงียบ) และ `index.html` ตกไปที่ fallback `kind: names` · ทั้งสองไฟล์มีคนอื่นถืออยู่ |
| 2c | ปลดเพดาน "2–4 เครื่อง" ที่เหลือ | **ยังไม่ทำ** — `infer_topology()` รับถึง 8 แล้ว แต่ข้อความช่วยเหลือของ `lmds cluster inspect/plan/apply` (`cli/main.py`), ตัวเลือกใน `index.html` และ `nodes/doctor.py` ยังเขียน/จำกัดไว้ที่ 2–4 |
