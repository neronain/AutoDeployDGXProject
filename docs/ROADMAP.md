...
| M7b ⏳ | Hardware validation (รอรันบนเครื่องจริง) | acceptance บน DGX Spark + RTX PRO 4000 ×2 + 4070: download → verify-files → start → test-text กับโมเดลอ้างอิง 5 ตัว, regression เทียบ controllers v3.0.0 เดิม, ปรับค่าคงที่ fit จากผลจริง | เกณฑ์สำเร็จ MVP ด้านล่าง |
