...
กล่องแชทที่ตอบจากสถานะที่แคชไว้ตอบคำถามที่คนถามบ่อยที่สุดไม่ได้ — "ทำไมตัวนี้ไม่ขึ้น"
มีคำตอบอยู่ใน log ของเครื่องปลายทาง ไม่ได้อยู่ในสรุปสถานะ

| ID | ข้อกำหนด | Priority |
|---|---|---|
| FR-1c.1 | ก่อนตอบ ระบบเลือกชุด **probe (อ่านอย่างเดียว)** จากแคตตาล็อกแล้วรันบนเครื่องที่เกี่ยวข้องผ่านทางเดิมของ `lmds node` — log ของ controller, GPU + process ที่ถือ VRAM, RAM/swap, ดิสก์, พอร์ต, docker, เน็ตเวิร์ก/fabric, `lmds doctor` | P0 |
| FR-1c.2 | **LLM ต้องเขียนคำสั่งเองไม่ได้** — เลือกได้แค่ชื่อรายการในแคตตาล็อก + พารามิเตอร์ที่ผ่าน validation แล้วโค้ดประกอบคำสั่ง (กติกาเดียวกับ Deployment Plan §8.2) | P0 |
| FR-1c.3 | หน้าเว็บต้องแสดงว่า **ไปดูอะไรมาบ้าง** และผลของแต่ละอันสำเร็จหรือล้ม — ผู้ใช้ต้องแยกออกว่าคำตอบมาจากการไปดูจริงหรือจากแคช | P0 |
| FR-1c.4 | งานที่ **เปลี่ยนสภาพเครื่อง** (restart, เปลี่ยน context/port/bind/gpu-util, ล้างแคช) เสนอได้แต่ลงมือเองไม่ได้ · ผู้ใช้เลือกจากเมนู **แก้เลย / ทีละขั้น / ยังไม่ทำ** ต่อหนึ่งงาน | P0 |
| FR-1c.5 | ตั๋วอนุมัติออกโดย**เซิร์ฟเวอร์** และเดินได้ต่อเมื่อเบราว์เซอร์ของผู้ใช้ส่งกลับมาพร้อมโหมด — LLM ออกตั๋วให้ตัวเองไม่ได้ · ตั๋วหมดอายุ และแต่ละขั้นใช้ได้ครั้งเดียว | P0 |
| FR-1c.6 | โหมด "ทีละขั้น" หยุดให้ดูผลทุกขั้น และ **ขั้นที่ล้มหยุดขั้นที่เหลือ** — ขั้นถัดไปตั้งอยู่บนสมมติฐานว่าขั้นก่อนหน้าสำเร็จ | P0 |
| FR-1c.7 | ผลจาก probe ถูก **redact** ก่อนเข้า prompt (log จริงมี key/endpoint ปนมาได้) และถูกตัดตามงบ เพื่อไม่ให้เบียดคำถามของผู้ใช้ | P0 |
| FR-1c.8 | ผู้ช่วยต้องมี **วิธีคิดของระบบนี้** ติดตัว (playbook: หลักฐานชนะความจำ, knob มาก่อนแก้ไฟล์, อาการแบบไหนเริ่มดูตรงไหน) และ **ค้นเอกสารจริงใน `docs/`** แทนการฝังสำเนาไว้ใน prompt ซึ่งจะล้าสมัยเงียบ ๆ | P1 |
| FR-1c.9 | ใช้ `complete_json` ที่ provider ทุกเจ้ามีเหมือนกัน ไม่ผูกกับ function calling ของเจ้าใดเจ้าหนึ่ง — ไม่งั้นใช้กับ vLLM/Ollama ในบ้านไม่ได้ | P0 |
| FR-1c.10 | ทุกอย่างในขั้นนี้ล้มได้โดยไม่ทำให้แชทตาย — ไม่มี provider / provider ล่ม / เครื่องต่อไม่ติด = ตอบจากสถานะแคชเหมือนเดิม | P0 |
| FR-1c.11 | เอกสารที่ผู้ช่วยค้นต้อง**เดินทางไปกับแพ็กเกจ** — เครื่องที่ติดตั้งด้วย pip ไม่มี repo ให้ค้น | P0 |
...
7. **ผลจากเครื่องปลายทางเป็น untrusted input เหมือน model card**: log ของ container และข้อความ error เดินทางเข้า prompt ของผู้ช่วย · ด่านที่กันไม่ใช่ถ้อยคำใน prompt แต่เป็นโครงสร้าง — ผู้ช่วยเลือกได้เฉพาะชื่อรายการในแคตตาล็อก พารามิเตอร์ผ่าน validation ก่อนถูกประกอบเป็นคำสั่งด้วยโค้ด และงานที่เปลี่ยนสภาพเครื่องยังต้องรอคนกดปุ่ม (FR-1c.2, FR-1c.5) · redact ก่อนส่งออกไปหา provider (FR-1c.7)
...
| 2026-08-06 | **`remove` ข้ามเครื่องทำได้ แต่ต้องสองขั้น** | ตอนแรกกันไม่ให้ทำจากหน้าเว็บเลย แต่ผู้ใช้ต้องไป ssh เองอยู่ดีซึ่งไม่ปลอดภัยกว่า · คำขอแรกรัน `--dry-run` เห็นรายการ+ขนาดจริง แล้วต้องส่ง `confirm` ที่ตรงกับ slug เป๊ะถึงจะลบ (`yes`/`true` ไม่ผ่าน) — ปุ่มที่กดพลาดจึงลบ weight หลายสิบ GB ไม่ได้ |
| 2026-08-06 | **cluster IP อยู่กับเครื่องของมัน ไม่ใช่ตารางรวม** | ผู้ใช้จริงรายงานว่าตารางรวมท้ายหน้าอ่านแล้วไม่รู้ว่าแถวไหนของเครื่องไหน · ย้ายเข้าการ์ดของเครื่อง + จัดเครื่องกลุ่มเดียวกันให้ติดกันแล้วคร่อมด้วยรั้วสี · **ลบตารางเดิมทิ้ง** เพราะข้อมูลเดียวกันอยู่สองที่คือต้นเหตุความสับสน ไม่ใช่ความสะดวก |
| 2026-08-06 | **สีมีได้ แต่ห้ามแย่งความหมาย** | ผู้ใช้ขอให้หน้าไม่จืด · ใช้ตระกูลสีเดียวไล่เฉดทั้งหน้า (ฟ้า→คราม→ม่วง→ชมพู) แทนสีทึบคนละที่ · **สีที่มีความหมายยังแยกจากสีตกแต่งชัดเจน** (เขียว/เหลือง/แดง = ปกติ/ใกล้เต็ม/เต็ม) · แสงพื้นหลังถูกลดลงหลังผู้ใช้รายงานว่าพื้นไปเสมอกับช่องกรอกจน "ดูพลาดได้" — ช่องกรอกเปลี่ยนเป็นผิวจมลง + ขอบเข้มกว่าการ์ด เพราะนั่นคือเรื่องกรอกผิดช่อง ไม่ใช่เรื่องความสวย |

| 2026-08-06 | **bundle เดินทาง ไม่ใช่คำสั่ง deploy** | เครื่องที่เปิดหน้าเว็บมักไม่ใช่เครื่องที่รันโมเดล · เลือกส่ง **ZIP ที่ผู้ใช้ตรวจแผนแล้ว** (`lmds node push`) แทนการสั่งให้ปลายทาง `lmds deploy` เอง เพราะปลายทางจะวางแผนใหม่ซึ่งอาจได้คนละค่า — อนุมัติแผนหนึ่งแล้วได้อีกแผนหนึ่งไปรัน |
| 2026-08-06 | **โมเดลบนเครื่องอื่นต้องคุมได้เท่ากับในเครื่อง** | ผู้ใช้ถามว่าทำไม manage สองฝั่งต่างกัน — ไม่มีเหตุผลทางเทคนิค เป็น controller ตัวเดียวกันและรับ env ชุดเดียวกัน · เปลี่ยน node จาก flag มาเป็น env และตรวจค่าด้วยฟังก์ชันเดียวกันทั้งสองทาง จะได้ไม่มีสองมาตรฐานให้ต้องตามซิงก์ |
| 2026-08-06 | **ชุดทดสอบต้องยืนยันว่าใครตอบ** | ทุก bundle ตั้งต้นที่พอร์ต 8000 · `test-text` ของโมเดลหนึ่งได้คำตอบจากอีกโมเดลที่รันอยู่บนพอร์ตนั้นแล้วรายงานว่าผ่าน · controller อ่าน `/v1/models` ก่อนทดสอบเสมอ ชื่อไม่ตรง = หยุด · **ผลทดสอบที่เชื่อไม่ได้แย่กว่าไม่มีผลทดสอบ** |
...
2. ~~**License โปรแกรม**: ขายลูกค้า (proprietary) หรือ open-source บางส่วน?~~
   → **ตัดสินแล้ว — กำลังทบทวน** (อัปเดต 2026-09-20) · `LICENSE` + `NOTICE` ในรีโปประกาศว่า
   LMDS เป็น **proprietary** · ส่วน **bundle ที่ generate ออกมาเป็นของผู้ใช้** (รันและเก็บได้)
   แต่ตัว generator ไม่ใช่ · โปรเจกต์พี่น้อง **LiteGate เป็น MIT** แยกต่างหาก ·
   **ถ้อยคำกำลังถูกแก้อยู่ตอนนี้** (ร่างอยู่ที่ `docs/legal-draft/`) — **อย่าอ้างเงื่อนไขจากหัวข้อนี้**
   ให้อ่าน `LICENSE` / `NOTICE` ที่ราก repo เป็นตัวจริงเสมอ
