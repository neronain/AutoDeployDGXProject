...
- **เตือนเรื่องการเดินสาย ConnectX ด้วย** (ชุดเดียวกับ `lmds node cluster`): ลิงก์ขึ้นแต่ไม่มี IP ·
  คู่แฝดอยู่วง subnet เดียวกัน · ไม่มี RoCE device คู่กัน · local mesh candidate ที่ยังไม่ได้ยืนยัน
  topology/OOB ข้ามทุก node
...
- **token อยู่ยาว** — เก็บที่ `~/.config/lmds/web-token` (0600) ใช้ซ้ำทุกครั้งที่เปิด ลิงก์จึง
  bookmark ได้ · เปลี่ยนด้วย `--new-token` หรือ `--token` เท่านั้น
