...
- **ข้อจำกัดที่ต้องรู้ก่อนใช้**: ต้องมี torch ที่เห็น CUDA อยู่บนเครื่องนั้น **เครื่องที่รัน llama.cpp อย่างเดียวจึงมักวัดไม่ได้**
  เพราะไม่เคยต้องมี image ของ vLLM · วัดจริงบนฟลีต 16 เครื่อง (2026-09-21): torch+CUDA บนเครื่องจริงมีเครื่องเดียว ·
  ที่เหลือวัดได้เพราะยืม image ของ vLLM · **3 เครื่องวัดไม่ได้เลย** เพราะไม่มีทั้งสองอย่าง · ผลคือ `kind: unknown` +
  `ok: true` (ไม่ใช่ "ตก") พร้อมทางแก้ · ปิดช่องว่างแบบไม่ต่อเน็ตได้ด้วย `docker save` จากเครื่องข้าง ๆ แล้ว `docker load`
  หรือชี้เองด้วย `LMDS_BURN_PYTHON` / `LMDS_BURN_IMAGE`
...
tests/                   # 149 ไฟล์ test_*.py · 2,451 เทส (unit + E2E + review/audit + JS shell ใน node) · addopts = -q
