...
| # | เรื่อง | สถานะ (0.9.0 — 2026-09-21) |
|---|---|---|
| 1 | `LMDS_REPO_REF` ปักหมุด tag | ✅ ปิดแล้ว · ดู §4 |
| 2 | LMDS ยังไม่มี **auth/RBAC/audit** | ◐ auth ✅ · audit ✅ (`lmds audit`) · **RBAC ยังไม่มี** — ดูด้านล่าง |
| 3 | model server ออกมาเป็น `0.0.0.0` **ไม่มี API key** | ✅ bundle ที่ deploy ใหม่ได้ key ตั้งแต่เกิด · ของเดิมไม่ถูกแตะโดยตั้งใจ |
| 4 | `source_bundle()` hardcode `main` | ✅ แพ็ก HEAD ของ hub แล้ว |
| 5 | LMDS ไม่มี lint gate ใน CI | ✅ job `ruff` ใน CI |

### ข้อ 2 ที่ยังเหลือ — RBAC

วันนี้เป็น **token เดียวแบบมี/ไม่มีสิทธิ์** ไม่ใช่ "ใครเป็นใคร" · `lmds audit` ตอบได้ว่า
*IP ไหนสั่งอะไรเมื่อไหร่* ซึ่งพอสำหรับการสืบย้อน แต่ไม่ใช่บัญชีผู้ใช้รายคน

ชั้น read-only (ให้คนดู dashboard ได้โดยสั่งอะไรไม่ได้) ทำพ่วงไม่ได้ ต้องไล่ตรวจทุก GET
ก่อนว่าอันไหนคืนความลับ — `logs` ของโมเดลผ่าน `redact()` ไม่ครบทุกทาง ปล่อยผ่านแล้ว
เท่ากับแจก log ที่อาจมี token ให้คนที่ตั้งใจให้ดูอย่างเดียว · เป็นงานของตัวเอง

### ข้อจำกัดที่รู้ตัว — bundle ที่ adopt มา

`lmds key` เติม key ให้ได้เฉพาะ container ที่มีตัวแปรชื่อ `*API_KEY*` อยู่แล้ว · container
ที่ไม่มีเลย (เช่น vLLM ที่รันโดยไม่ตั้ง `VLLM_API_KEY` ตั้งแต่แรก) ต้องตั้ง auth ที่คำสั่ง
ของ engine เอง แล้ว adopt ใหม่ · สคริปต์ที่ generate ออกมาเขียนเรื่องนี้ไว้ตรง ๆ และ
`lmds doctor` ขึ้น `endpoint` เป็น WARN ให้ ไม่ขึ้นเขียวหลอก
