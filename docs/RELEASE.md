...
`lmds fleet check` ไม่ SSH โดยปริยาย — ตัวเลขมาจากทะเบียนซึ่งเป็นของรอบที่ probe ล่าสุด
หลัง `node install` เสร็จจึงยังเห็นเวอร์ชันเก่าได้ · ตารางบอกอายุของข้อมูลรายแถว
(คอลัมน์ "ข้อมูลเมื่อ") และขึ้นบรรทัดเตือนเมื่อยังไม่ได้ต่อเข้าเครื่องจริง

```bash
lmds fleet check            # เร็ว ไม่ SSH — ของที่ทะเบียนจำไว้
lmds fleet check --check    # ต่อเข้าทุกเครื่องพร้อมกัน + เขียนผลกลับทะเบียน
```
