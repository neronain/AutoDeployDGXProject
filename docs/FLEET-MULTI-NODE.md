...
| หาอะไรอยู่ | ไปที่ |
|---|---|
| เพิ่มเครื่องเข้าทะเบียน · เปลี่ยน hostname ของเครื่อง | [3. เพิ่มเครื่องเข้าทะเบียน](#3-เพิ่มเครื่องเข้าทะเบียน) |
| สั่งงาน/ดูสถานะข้ามเครื่อง | [4. ดูและสั่งงานข้ามเครื่อง](#4-ดูและสั่งงานข้ามเครื่อง) |
| เครื่องไหน stacked ด้วยกันได้ · ต่อสาย ConnectX · `cluster pair` / `doctor` | [5. Cluster fabric — เครื่องไหน stacked ด้วยกันได้](#5-cluster-fabric--เครื่องไหน-stacked-ด้วยกันได้) |
| เข้าถึงจากนอกออฟฟิศ · ทำสำเนาโมเดลข้ามเครื่อง | [5.1 เข้าถึงจากนอกออฟฟิศ — ที่อยู่สำรอง](#51-เข้าถึงจากนอกออฟฟิศ--ที่อยู่สำรอง) |
| อ้างอิงคำสั่งทั้งหมด | [8. อ้างอิงคำสั่งทั้งหมด](#8-อ้างอิงคำสั่งทั้งหมด) |
| ทุกเครื่องรันโค้ดชุดเดียวกันจริงไหม · "ตรง hub" คืออะไร | [ตรวจว่าทุกเครื่องรันโค้ดชุดเดียวกันจริง](#ตรวจว่าทุกเครื่องรันโค้ดชุดเดียวกันจริง) |
| bundle เก่า · อัปเดต llama.cpp บน node | [bundle เก่า — Update regenerate ให้เอง (ออฟไลน์)](#bundle-เก่า--update-regenerate-ให้เอง-ออฟไลน์) |

ลำดับคำสั่ง stacked ที่รันจริงแล้ว พร้อมเวลาและตัวเลขของจริง: [RUNBOOK-MULTI-NODE.md](RUNBOOK-MULTI-NODE.md)
...
### เปลี่ยน hostname ของเครื่อง (0.8)

เครื่องที่ส่งออกไปอยู่กับลูกค้าแล้วพบว่า **ตั้ง hostname ซ้ำกัน** แก้เองไม่ได้ เพราะเข้าถึงได้ทาง
**คอนโซล LMDS (พอร์ต 8600) ทางเดียว** ไม่มี SSH ให้ใครเข้าไปพิมพ์ `hostnamectl` — จึงทำจากที่นี่ได้:

**หน้าเว็บ** (ทางหลัก): การ์ดเครื่อง → ปุ่ม **Rename host** → กรอกชื่อใหม่ + รหัส sudo แล้วกด Rename ·
ฟอร์มโชว์ชื่อปัจจุบัน ชื่อที่ถูกใช้ไปแล้วทั้งฟลีต และผลทีละขั้น · ช่องรหัสไม่ขึ้นบนเครื่องที่ sudo เป็น NOPASSWD

```bash
lmds node rename-host msi-6 spark-7       # CLI ทางเดียวกัน (ถามรหัส sudo ตอนนั้น ใช้ครั้งเดียว)
```

**ชื่อในทะเบียนกับ hostname ของ OS เป็นคนละอย่าง — คำสั่งนี้เปลี่ยนอย่างหลัง**

| | คืออะไร | เปลี่ยนยังไง |
|---|---|---|
| ชื่อในทะเบียน (`nodes.yaml` → `name`) | ป้ายที่ **hub** ใช้เรียกเครื่องนี้ — `lmds node run <ชื่อ>`, ปุ่มบนหน้าเว็บ, ลำดับการ์ด, สมาชิกกลุ่ม stacked, `CLUSTER_NODES` ใน cluster.env ที่เขียนไปแล้ว | `remove` แล้ว `add` ใหม่ — เหมือน host/user/port ที่ `node set` ไม่ให้แก้ เพราะมันคือ *ตัวตนของแถว* ไม่ใช่ข้อมูลในแถว |
| hostname ของ OS | ชื่อที่ **ตัวเครื่อง** เรียกตัวเอง — prompt, journal, ป้าย hostname ในหน้า Cluster | `lmds node rename-host` / ปุ่มบนหน้าเว็บ |

ที่ลูกค้าเจอซ้ำกันคือ hostname ของ OS ล้วน ๆ (ทะเบียนกันชื่อซ้ำอยู่แล้วตอน `add`) — และ **ไม่ใช่เรื่อง
การนับเครื่องตามไลเซนส์**: `machine_identity()` ผูก hostname เข้ากับชุด IP บนสายเร็วอยู่แล้ว เครื่องคนละตัว
ที่ชื่อซ้ำกันจึงถูกนับแยกถูกต้องมาตลอด · ที่พังคือคนอ่าน

**สิ่งที่คำสั่งนี้ทำให้ด้วย**

- แก้ `/etc/hosts` (บรรทัด `127.*` ที่อ้างชื่อเดิม รวม `<ชื่อ>.local`) ใน **คำสั่ง sudo เดียวกัน** —
  ไม่แก้ = `sudo` ทุกครั้งขึ้น `unable to resolve host` และช้าลงเป็นวินาที ซึ่งไม่มีอะไรบอกว่ามาจากตรงนี้
- สำรอง `/etc/hostname` + `/etc/hosts` ไว้ที่ `/root/lmds-hostname/*.<stamp>` (ไม่ลบ ทางเดียวกับ
  `netplan` ที่เก็บไฟล์ไว้ที่ `/root/netplan-disabled`) · ล้มกลางคัน = ถอยกลับให้เอง · ถอยด้วยมือก็ได้
- สั่ง hub สำรวจเครื่องนั้นใหม่ ป้ายบนจอจะได้ไม่ค้างชื่อเก่า

**สิ่งที่ไม่ต้องแก้ตาม** (ตรวจจากโค้ดแล้ว): `cluster.env`/NCCL/controller ใช้ **IP** ล้วน
(`MASTER_IP`, `WORKER_IPS`, `TRANSPORT_IP_*`, `--master-addr`) · `server.meta`, `MODEL_PROFILE.yaml`,
`bundle.env`, ไฟล์ netplan ไม่มี hostname อยู่ในนั้นเลย · `hostname` ในสคริปต์ controller คือ `hostname -I`
(หา IP) ไม่เปลี่ยนตามชื่อ · ผลวัด `~/.lmds/bench/*.json` จดชื่อ ณ วันที่วัดไว้เป็นหลักฐาน **ตั้งใจไม่แก้ย้อนหลัง**

**ที่ปฏิเสธ ไม่ใช่ทำครึ่งเดียว**

- ชื่อใหม่ซ้ำกับ hostname ของเครื่องอื่น ซ้ำกับชื่อในทะเบียนของเครื่องอื่น หรือซ้ำกับ hub
  (hub ไม่มีแถวในทะเบียน ชื่อสมาชิกคลัสเตอร์ของมันคือ hostname ตรง ๆ) — เครื่องเดียวที่ถูก add ไว้
  สองชื่อ **ไม่นับว่าซ้ำ** (ยุบด้วย `machine_identity` ตัวเดียวกับที่ใช้ยุบฝาแฝดอยู่แล้ว)
- hub เข้าเครื่องนั้นด้วย *ชื่อที่กำลังจะเปลี่ยน* (เช่น `lmds node add spark1.local`) — เปลี่ยนเสร็จก็เข้าไม่ได้อีก
  และไม่มี SSH ให้ไปแก้คืน · ให้ `remove` แล้ว `add` ใหม่ด้วย IP ก่อน
- มีโมเดล **stacked** รันอยู่ (ฝั่ง head หรือ worker) — rendezvous ของ NCCL กับ `/etc/hosts` กำลังถูกใช้อยู่
  และจาก hub ยืนยันไม่ได้ว่ากลุ่มยังดีหลังเปลี่ยน · หยุดที่ head ก่อน
- ชื่อผิดกติกา RFC 1123 (a-z 0-9 `-` ไม่เกิน 63 ตัว ไม่ขึ้น/ลงท้ายด้วย `-` ไม่มีจุด) — ตัวพิมพ์ใหญ่
  ไม่ใช่ความผิด ระบบลดเป็นตัวพิมพ์เล็กให้แล้วบอกว่าตั้งชื่ออะไรไปจริง

> คู่ที่จับกุญแจ stacked ไว้แล้ว: `~/.ssh/config` บน head มีบรรทัด `Host` ที่ลิสต์ชื่อเครื่องไว้ด้วย
> (นอกเหนือจาก IP) · controller ต่อด้วย IP อยู่แล้วจึงไม่พัง แต่ถ้า head→worker ssh บ่นเมื่อไร ให้รัน
> `lmds cluster pair` ซ้ำ — ระบบเตือนไว้ในผลลัพธ์อยู่แล้ว
...
- **ชื่อในทะเบียนไม่จำเป็นต้องตรงกับ hostname** — เป็นคนละอย่างโดยตั้งใจ (ทะเบียนชี้ที่
  ...
  แก้ hostname ของ OS ได้จากที่นี่: `lmds node rename-host` / ปุ่ม **Rename host** บนการ์ดเครื่อง (ข้อ 3)
