...
### ต้องเตรียมอะไรบนเครื่องปลายทาง

1. ติดตั้ง LMDS แล้ว (`lmds` อยู่ใน PATH ของ user นั้น)
2. user ที่ใช้ login **อยู่ในกลุ่ม `docker`**
3. sshd เปิดอยู่
...
| `slow` | ตั้งไปบนเส้นที่ช้าเกินไป (มัก = เผลอใส่ IP ของสาย 1G) |
...
- ยังไม่ได้ทดสอบ fabric detection กับ ConnectX บนเครื่องจริง — logic มีเทสครอบด้วย sysfs จำลอง
  ค่าที่อ่านจากเครื่องจริงต้องยืนยันอีกรอบเมื่อมี DGX Spark 2 เครื่องพร้อมกัน
