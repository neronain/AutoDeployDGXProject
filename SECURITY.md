...
**ทำจากหน้าเว็บได้ด้วย** — ปุ่ม **Key** บนการ์ดโมเดล · `lmds key` เป็น CLI ล้วนมาตลอด ขณะที่ลูกค้าส่วนใหญ่
ใช้ GUI อย่างเดียว (และ wizard deploy บนหน้าเว็บเองยังเคยพิมพ์บอกให้ไปรัน `lmds key show --reveal`
ซึ่งเป็นคำสั่งที่คนกลุ่มนั้นพิมพ์ไม่ได้) · กติกาของ endpoint ชุดนี้:

| | |
|---|---|
| `GET /api/models/{slug}/key` | คืนแค่ *มี/ไม่มี* + hint 4 ตัวหน้า-หลัง — **ไม่คืนค่าเต็ม** เพราะหน้าเว็บ poll ตลอดเวลา key จะไปนอนใน memory ของเบราว์เซอร์โดยไม่มีใครขอ |
| `GET …/key/reveal` | แยก endpoint ต่างหากสำหรับ "ดูค่าเต็ม" เพื่อให้แยกได้ว่าใครกดดู |
| `POST …/key` | ไม่ส่งค่ามา = สุ่มให้ · **ทับของเดิมต้องส่ง `force`** ไม่งั้น 409 (client ทุกตัวที่ถือใบเก่าใช้ไม่ได้ทันทีที่ restart) |
| `DELETE …/key` | ถอนออก — ตอบกลับพร้อมคำเตือนว่าหลัง restart จะเสิร์ฟแบบเปิด |

⚠️ **การ *อ่าน* key ไม่โผล่ใน audit** — middleware เก็บเฉพาะคำสั่งที่เปลี่ยนสถานะกับคำขอที่ถูกปฏิเสธ ·
ยอมรับข้อนี้โดยตั้งใจ เพราะคนที่ถือ token ของคอนโซลอ่านอะไรก็ได้อยู่แล้ว และการเก็บทุกการอ่าน
จะกลบคำสั่งที่เปลี่ยนของจริงจนหาไม่เจอ
...
| hostname ใหม่ (`node rename-host` / ปุ่ม Rename host) | ตรวจ RFC 1123 ก่อนถึง shell · `hostnamectl set-hostname` + `/etc/hosts` อยู่ในคำสั่ง sudo เดียว · **รหัส sudo ทาง stdin เท่านั้น** ไม่อยู่ใน argv/log/ทะเบียน · สำรองไฟล์เดิมไว้ที่ `/root/lmds-hostname/*.<stamp>` และถอยกลับเองเมื่อล้มกลางคัน · ปฏิเสธเมื่อ hub เข้าเครื่องนั้นด้วยชื่อที่กำลังจะเปลี่ยน (เปิดแล้วปิดไม่ได้ — เครื่องกลุ่มนี้ไม่มี SSH ให้ไปแก้คืน) มีโมเดล stacked รันอยู่ หรือชื่อใหม่ชนกับเครื่องอื่น/กับ hub |
...
เดิม token ถูกสุ่มให้เฉพาะตอน bind ออก network · `127.0.0.1` ซึ่งเป็นค่าปริยายถูกปล่อยโล่งโดยถือว่า
"เครื่องนี้เท่านั้น" = ปลอดภัย ซึ่งไม่จริงสองทาง: **ผู้ใช้อื่นบนเครื่องเดียวกัน**เปิด `localhost:8600`
แล้วได้สิทธิ์ของ user ที่รัน hub (คุม start/stop/ลบโมเดลได้ทุกเครื่องในทะเบียนผ่าน SSH) และ
**เพจใดก็ได้ที่ผู้ใช้เปิดในเบราว์เซอร์**ยิง `POST` มาที่ `127.0.0.1` ได้ (CSRF / DNS rebinding) —
token ปิดทางหลังเพราะเพจคนละ origin อ่าน `localStorage` ของหน้านี้ไม่ได้

ตอนนี้ `lmds web` ตั้ง token ให้เสมอไม่ว่า bind ที่ไหน · `--no-auth` เปิดโล่งได้แต่ต้องสั่งเอง
และพิมพ์คำเตือนสีแดงทุกครั้ง · token เดินทางไปยัง process เบื้องหลังทาง **environment ไม่ใช่ argv**
(`ps` อ่าน argv ของทุก process บนเครื่องได้ ส่วน `/proc/<pid>/environ` อ่านได้เฉพาะเจ้าของกับ root)
· ไฟล์ที่ถือ token — `web-token`, `web.json`, unit ของ systemd — ถูกสร้างด้วยโหมด `0600`
ตั้งแต่ open ไม่ใช่เขียนก่อนแล้ว `chmod` ทีหลัง (ระหว่างสองขั้นนั้นไฟล์อ่านได้ทั้งเครื่อง)

### ร่องรอยการใช้งาน (audit)

```bash
lmds audit              # 40 รายการล่าสุด
lmds audit --failed     # เฉพาะที่ถูกปฏิเสธ — ไล่เดา token จะเห็นเป็นชุดจาก IP เดียวกัน
```

`~/.lmds/audit.log` โหมด 0600 · เก็บคำสั่งที่เปลี่ยนสถานะ (POST/PUT/DELETE/PATCH) กับคำขอที่ถูกปฏิเสธ
(401/403/429): เวลา · IP · method+path · ผลลัพธ์ · เวลาที่ใช้ · เก็บที่ middleware จุดเดียว route ใหม่
ที่ลืมใส่จึงไม่มี · **ไม่เก็บ body และไม่เก็บ query string** เพราะ token เดินทางมาทาง `?token=` ได้
· หมุนไฟล์ที่ 5 MB เก็บรุ่นเดียว · `$LMDS_AUDIT=0` ปิด · `$LMDS_AUDIT_LOG` ย้ายที่

หน้าเว็บอ่านชุดเดียวกันได้จากแผง **"ใครสั่งอะไร"** ในหน้า *Hub settings* (`GET /api/audit`) พร้อมตัวกรอง
"เฉพาะที่ถูกปฏิเสธ" — ลูกค้าที่เปิดคอนโซลออกวง network ต้องเห็นเองว่ามีใครพยายามเข้า ซึ่ง `lmds audit --failed`
ทำได้อยู่แล้วแต่คนที่ใช้ GUI อย่างเดียวเข้าไม่ถึง

**สิ่งที่ระบบสั่งเองก็ลงที่นี่** — `lmds watchdog` restart โมเดลเมื่อ generate จริงไม่ตอบติดกันหลายรอบ
ทุกครั้งถูกบันทึกพร้อมเหตุผล เพราะคำถามแรกเวลามีอะไรผิดปกติคือ "ใครสั่ง" และคำตอบ "ระบบสั่งเอง"
ต้องหาเจอที่เดียวกับคำสั่งของคน · watchdog เป็น **opt-in ต่อ slug** ไม่มีอะไรเปิดเอง และปฏิเสธ
container ที่ LMDS ไม่ได้สร้าง

**ยังไม่มี: บัญชีผู้ใช้รายคนและ role** — วันนี้เป็น token เดียวแบบมี/ไม่มีสิทธิ์ ไม่ใช่ "ใครเป็นใคร"
ชั้น read-only ต้องไล่ตรวจทุก GET ก่อนว่าอันไหนคืนความลับบ้าง (`logs` ของโมเดลผ่าน `redact()`
ไม่ครบทุกทาง) ซึ่งเป็นงานของตัวเอง ไม่ใช่ของที่ทำพ่วงมากับเรื่อง auth
